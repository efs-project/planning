...
| `open-decisions.sh` | Regenerate `Open-Decisions.md` — the roll-up of every `Designs/**/owner-decision-inbox.md`, holds first and loudest. Carries an item's optional `**Reply forms:**` line into its question row. Warns on stderr (and exits 1) if an item falls under an unrecognized section or a queue may have an undetected hold. | In the same commit as any decision-state change. |
