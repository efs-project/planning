...
Not blocking. A candidate for a dedicated brainstorm/design (likely an SDK + lens-model concern). **Update 2026-07-01:** the *burner-session* half **shipped** — contracts **PR #39 "instant Sepolia burner session"** merged to `main` (chain-aware burner + network persistence). The **multi-wallet-as-one-identity-in-lenses** half — wallet *lists* as lens arguments so a user's combined addresses resolve as one identity — is **still open** and is the durable part worth designing (ties to ARCH-9 key-rotation + the SDK identity model).
