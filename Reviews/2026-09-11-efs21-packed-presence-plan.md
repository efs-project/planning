...
Controller scope clarification: the existing three-arm measurement test may validate its retained source-pinned evidence instead of launching the unrelated original-storage benchmark in every full Node run. Retain its substantive assertions and label it historical evidence validation. The old body-world integrity test stays live, and the new packed two-arm test must freshly exercise the complete matched workload against frozen58/current. Keep the runner's default three-arm behavior unchanged and disclose this test maintenance; do not present historical fixtures as fresh receipts.
