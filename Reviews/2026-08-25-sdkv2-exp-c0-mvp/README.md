...
no Core implementation, generator, script, or test. Using these five retained
files, the SDK semantic fixture and the prerequisites below, it independently:
...
This retained checker requires Node and Git. It resolves ethers through the
sibling `../contracts/package.json` relative to the planning root, so that
checkout must have its ethers dependency installed. It also requires a sibling
`../planning-v2-readiness` Git checkout containing exact commit
`b9088d6a24f4d40bcca6ba300523b25cc7c608d2`. Read-only Git commands compare the
five retained artifacts against that commit; the current checkout branch is
not used as the source of truth. No Core implementation or tests are imported.
This is a local source-verification harness, not a standalone reproduction
bundle.

Run from `Reviews/2026-08-25-sdkv2-exp-c0-mvp/` inside the planning checkout:
