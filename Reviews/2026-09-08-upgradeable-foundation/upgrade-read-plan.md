...
- Parent owns design/status/evidence docs and the task/final reviews. Task 1 worker owns its nine enumerated source/test paths and sole Core+upgrade build window until report; Task 2 owns only its specified test-configuration line in a later window. No worker push or subagents.
...
- Modify `Reviews/2026-09-09-files-parity-performance/comparison.test.mjs`: explicitly pin the reviewed base/reads runner migration while retaining the frozen historical reports and all other comparison gates. See integration refinement below.
...
**Integration refinement (September9):** the first broad run passed188/189 Node tests; the sole failure was the old comparator's all-support-pins equality at comparison.test.mjs:29. The required runner extension necessarily changes that one pin. Permit exactly its frozen old hash and the explicitly pinned, reviewed new runner hash; never a wildcard, current-disk-derived allowlist, or ignored source. Every other support pin/key and every existing workflow/input/compiler/runtime-source, cleanup, semantic, calldata and gas assertion remains enforced. Keep baseline.json and optimized.json untouched. Add focused positive controls for old/new permitted runner pins and negative controls for unapproved/missing runner, missing/extra support keys and changed non-runner support pins. Label the current test/report as a base-profile regression after runner integration, not a newly isolated journal-only A/B. Any optimized-export metadata must identify the changed support source and preserve the original frozen comparison boundary; it must not falsely claim all support sources unchanged. No evidence export is requested in this task.
...
- [x] **Step 3: Build the fixed upgrade query library and host.** Query forwards reuse unchanged checked state/Lens functions. Project their current observation basis only after exact result computation. Raw/hydrated pages use Step2 before token work. Preserve existing return and error ABI; declare delegated errors on the host where needed. Guard dependencies before request/state errors. No getReceipt selector or fabricated support manifest.
...
- [x] **Step 5: Share only the pure oracle model.** Extract the existing declarative tier/group model, preserving original resolveLens tests. The upgrade wrapper reverifies supplied snapshot/expected components/history, finds exact admitted Plan bytes, parses them and uses the verified current executionId and block/H. Wrong/missing history, forged VERIFIED/fold, wrong read code or mixed pin never yields a verified target. Do not normalize revision fields or mutate old snapshots to pass the old verifier.
...
- [x] **Step 8: Full regression, self-review, exact commit and handoff.** Worker owns Core+upgrade artifact build window. Force AST/build-info builds when necessary without weakening provenance. Run both Forge suites and all Core/upgrade/Files/admission/type-input Node tests serially with all saved-evidence exports0. Preserve old JSON. Read the exact staged diff, run whitespace checks and commit exact nine paths via git commit -F with v2-pm/Codex/Astra trailers; never push. Report red/green evidence, commands/results, warnings, codehash/context-chain proof, source SHA, actual measurements and machine-readable scratch evidence. Parent performs independent task review, fresh integration and separate whole-increment review before experiment push.

## Task 2: Make the normal local test command reproducible

**File:** modify only `Reviews/2026-09-08-upgradeable-foundation/foundry.toml`.

The source task is reviewed at `bcd0643`; its contract and comparator sources
remain unchanged. Parent owns all documentation and the build window except
while this small task runs. No source/runtime, network or global permission
change is permitted.

- [x] **Red:** from the foundation directory, without FOUNDRY_CONFIG or other
  permission overrides, run the existing `test/UpgradeReads.t.sol` focused
  Forge tests with the installed compiler. Retain the out-artifact read refusal.
- [x] **Edit:** change only the fs_permissions assignment to exactly:

```toml
fs_permissions = [{ access = "read", path = "out" }, { access = "read", path = "../2026-09-05-mvp-build-start/type-inputs" }]
```

- [x] **Green:** inspect `forge config --json`: same source/test/out/libs,
  remappings, compiler, optimizer, viaIR and EVM values; exactly these two read
  permissions. Run focused tests and the full foundation Forge suite without
  overrides; require19/19 pass. Compare compiled deployed runtime objects
  before/after; no Solidity artifact bytes may change. Do not force rebuild or
  rerun unrelated Node/Core suites for this permission-only edit.
- [x] **Handoff:** self-review the one-line diff; commit only this TOML through
  git commit -F with actual model, role v2-pm and harness codex trailers. No
  push/subagents. Write red/green/effective-config/runtime-equality evidence
  and the exact commands to the designated report; return short status/SHA.
