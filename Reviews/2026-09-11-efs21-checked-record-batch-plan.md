...
Controller retry clarification: each explicit `getRecords` uses the existing fresh acquisition path, so an RPC reply that later fails batch-semantic checks is not retained in the scalar ABI cache. Same ordered IDs can be explicitly retried after a recoverable transport response; terminal context/source/cancel rules remain. Each attempt consumes real request/byte budgets and has its own evidence ID. No automatic retry or scalar fallback; Files-level successful assessed-item caching is a later consumer concern. Preserve existing scalar cache behavior.
