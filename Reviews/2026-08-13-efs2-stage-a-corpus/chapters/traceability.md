...
| C-TX-3 | History append-only; authenticated pre-withdrawal/withdrawal/revocation/tombstone/replacement never erase bytes or resurrect older values | [DERIVED INVARIANT — const.] | bind §3.4/§4; auth §3.2–3.3 Admission-only typed ValidatedOccurrenceLifecycleEffect to Index+Binding; idx §3 RAW_AUDIT history | GV-9, GV-18 | CV-WITHDRAW; CV-PREWITHDRAW; FX-EAP.C, FX-NANDA.D | KA-12 | COVERED |
...
| C-LN-2a | Bounded deterministic contract point-resolution profile with FOUND/ABSENT/CONFLICT/UNSUPPORTED/UNKNOWN | [DERIVED INVARIANT — const.] | lens §5–§7: indivisible `ResolvedTarget(targetKind,targetA,targetLeaf)`, combiners T1–T10, algorithm + Presence enum | GV-13 incl. same-envelope/different-leaf non-alias vectors | FX-LENS grid (V2-E2) | CF-8; AA-2 | COVERED |
