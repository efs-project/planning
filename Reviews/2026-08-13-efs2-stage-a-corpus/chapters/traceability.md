...
|---|---|---|---|---|---|---|---|
| C-UI-1 | IDs deterministic, domain-separated, versioned, generatable without mining a receipt/tx | [DERIVED INVARIANT — const. Universal identity; no-mined-UID direction, owner-rulings 2026-08-12] | enc §1.3/§1.6 (pinned domains + outer Codex bytes/hash and owner-module completeness), prin §3.8 (length-delimited authority module), idx §0.1 (length-delimited index module), enc §4 (ID family); profileId/genesis commit exact authority/query semantics without owner-local hashes | GV-3, GV-17 C-4/C-6 | all (ids precomputed offline in every script); H-DOMTABLE/H-MODULE-COMPLETE/CV-RECON | CF-1; GV-3 one-word sensitivity | COVERED |
...
| C-AA-2 | EOA and ERC-1271 authorship work in a fresh Realm without another chain's identity service; descriptor identity cannot be forged | [DERIVED INVARIANT — const.; V2-E5 gate text; SR-13/SR-14] | prin §3.4/§3.8 (`VerifierVM/1`: V1–V5 plus Principal equality in one ordered common prelude, then profile-committed fixed-schema opcode programs for exact EOA/7702, ERC-1271, P-256, and EFS-policy prehashed RSA behavior), §5 (1271/6492 policy); auth/realm write path consumes that byte-committed order | GV-5 | CV-AUTHCHAIN; FX-ARC (C2 = 1271 curator), FX-LENS cast | CF-1, CF-2; SR-13 mismatch attack | COVERED |
| C-AA-3 | Historical admission records authority + implementation basis; later account/controller/Core changes never silently reinterpret | [DERIVED INVARIANT — const.; kel admission-time lesson via CARRY-IN] | prin §3.5 (AuthorityBasis persisted; receipt interpretation, never current-code replay); auth §3.3/§4.3 (admission-time; accepted Withdrawal receipt/batch grounds T4); realm §7/§8 W-7 | GV-12; GV-5 (pinned-codehash receipt immutability); GV-17 | CV-7702; CV-PREWITHDRAW | CF-7; KA-5, KA-6 | COVERED |
...
| C-TX-2 | Identical-content retry idempotent; mutable state uses explicit predecessor/CAS | [DERIVED INVARIANT — const.] | SR-15 occurrence-granular no-op; auth §5.3 static revision association + guarded u48 Envelope allocation; realm §5.4/§5.5; bind §3.2–§3.5 sequential shadow head/source | GV-8 u48 boundary; GV-9, GV-18 | CV-SPARSE-ADMIT; CV-SHADOW; FX-GIT races | KA-4 | COVERED |
| C-TX-3 | History append-only; authenticated pre-withdrawal/withdrawal/revocation/tombstone/replacement never erase bytes or resurrect older values | [DERIVED INVARIANT — const.] | Admission/Authorship alone rejects a terminal selected source before effects/commit with `E_NO_RESURRECTION(bytes32 envelopeId,uint16 leafIndex)`; bind §3.4/§4 and idx §2.3/§3 consume typed/prevalidated lifecycle state and expose no alternate selector; every effective external T4 retains complete bounded TargetRecordCommitment evidence; RAW_AUDIT remains a sequential journal | GV-9, GV-18 | CV-WITHDRAW; CV-PREWITHDRAW/T4-MAX-BODY; CV-SHADOW; FX-EAP.C, FX-NANDA.D | KA-12 | COVERED |
...
| C-IX-2 | Baseline automatic reads: exact Types, Records, Occurrences, receipts, global admission order, unique-by-Type, Occurrences by Type/Record/Principal | [DERIVED INVARIANT — const.; 2026-08-12 "remain automatic"] | idx §3.1–§3.4; global order has only `admissionLogPage(PageRequest)->PageResult`, while `admissionAt` is point-only; SR-18a u16 digest key and SR-18d last-live count | GV-14; GV-3 (pk()) | CV-DIGEST-LOOKUP; CV-LAST-LIVE-COUNT; hf §3.1 read matrix/result registry | CF-14 (aggregate) | COVERED |
...
| C-IX-4 | Paginated typed backlinks by role/target; hot values, spam, dead history never turn point reads or locator selection into unbounded scans | [DERIVED INVARIANT — const.; owner ruling item A] | idx §0.1/§3.5/§5.1a/§7/§8; every B0 selector posting visited consumes the cap; one nonzero trusted Principal is context-bound and differently authored postings are skipped before scoring; SelectCursor context binds Realm + exact realmBasisAt(H) + target/spec while separate basisOrdinal binds H; all continuation grammar is byte-committed in the profile | GV-14 (first/middle/end; Principal/query/context corruption rejection including wrong Realm and same-H/different-realmBasis; unique outer/inner continuation) | WL-SPRAY, WL-HOT, WL-CHURN, WL-DEAD-LOCATOR | CF-5; dead/untrusted-spray total-visit bound | COVERED |
| C-IX-5 | Physical layout replaceable until frozen; the semantic query contract (typed keys, basis, cursor, coverage, completeness) is durable | [DERIVED INVARIANT — const., quoted verbatim in idx §0] | idx §0, §5/§5.1a (durable `uint256` page ABI and canonical PageCursorV1/UniqueTypeCursorV1 grammar); axis-7 arms swap physics under one ABI | GV-14 (ABI-level, arm-independent cursor words) | — | axis-7 verdict cannot change the ABI (bk §4.7) | COVERED |
| C-IX-6 | Adopted generic query outcomes (reverse membership/cited-by, digest lookup, author enumeration, revocation-aware counts, best-locator) remain costed gates; budget failure returns to James | [OWNER RULING — 2026-07-15 items A/B/C/D/E/13; 2026-08-12 obligation clause] | idx §3.4/§3.5/§6; idx §7 bounded trusted-Principal structural selector with explicit winnerPresent and `(0,0)` no-winner; cont §10.3 URI/content/health conformance and fallback remain the richer client tier | GV-14 zero-score/no-winner/Principal filter; corrected GV-16 split suites | CV-CLOCK; CV-DIGEST-LOOKUP; CV-LAST-LIVE-COUNT; WL-DEAD-LOCATOR; M-AGG | CF-14 → RETURN-TO-JAMES | COVERED at spec level; price is Stage B |
...
| ID | Requirement | Label | B0 realization | Vector | Fixture | Falsifier | Status |
|---|---|---|---|---|---|---|---|
| C-HR-1 | Every enumeration hard-bounded; exposes Realm, policy/code basis, high-water, cursor, coverage, COMPLETE/PARTIAL/UNSUPPORTED/UNKNOWN; UNKNOWN never absence | [DERIVED INVARIANT — const.; R-L6 register:205] | idx §5/§5.1a (PageResult plus versioned/reserved-checked, canonical-end/range-checked cursor grammar and deterministic PARTIAL progress); realm §4.2 (H-1); bind §6; hf §3.2 records the unchanged uint256 cursor with realmId/realmBasis/basis/highWater/coverage/completeness and canonical result digest | GV-14 (first/middle/end, context/corruption rejection, nested continuation, never-empty, one-basis, UNSUPPORTED members); vf §0.3 | every read row carries resultSchemaId/resultDigest, typedErrorCode, crossImplEqual, context fields, and exact N/A sentinels | CF-6 | COVERED |
| C-HR-2 | Canonical Type/Record/Occurrence/admission/index/current-fold bytes stay state-readable; never hash-only elision, event logs, or private DBs | [OWNER RULING — 2026-07-15 items 17 (full-body spine, "PAY IT") + 18 (no elision, "ETCH IT")] | enc §1.6 exposes exact Codex bytes containing prin §3.8 AUTHORITY + idx §0.1 INDEX modules; enc §2.6 full Record-body spine; idx §2/§2.4 log/meta words incl. guarded u48 Envelope inventory; enumerable AdmissionBatch intent lanes plus point nonce getter; lens §4.1 | GV-3 owner modules + GV-17 walk/C-4/C-6 | H-DOMTABLE/H-MODULE-COMPLETE; CV-RECON | CF-10 | COVERED |
| C-HR-3 | Second implementation reconstructs Types, Records, Occurrences, admissions, indexes, current folds from Realm state + byte carriers, no EFS service | [DERIVED INVARIANT — R-M2 register:146; const.] | realm §8 W-0..W-10 including W-4a + §8.2; batch walk reconstructs explicit nonce lanes; W-7 treats the accepted Withdrawal receipt/batch as historical validation fact and retained target bytes as identity preimages only, with no current/archive ERC-1271 call or required witness replay | GV-17 (two independent client rebuilds = the acceptance trace) | CV-RECON after every fixture; CV-PREWITHDRAW/T4-MAX-BODY; FX-GIT.E walk-away | CF-10; RECON_1 (hard gate, both axis-1 arms) | COVERED |
...
| C-FB-5 | Content identity independent of storage provider; Locators plural; shared-gateway ≠ independent custody | [DERIVED INVARIANT — const.; owner ruling item C for the selection view] | idx §7 owns B0 `B0_SELECT` (exact trusted Principal + structural/live/declared score only, total-visit bound); cont §2.1/§3/§10 owns URI/content/health validation, client fallback, and deferred TS/RS `SELECT_PROFILE_V2` | corrected GV-16 split suites | FX-ARC, FX-50GB; CV-CLOCK; WL-DEAD-LOCATOR | GV-16 claimed-time + dead/untrusted-spray members | COVERED |
...
| AT-5 | Type/scalar/backlink/set/Lens reads paginate at a pinned basis; truncation/missing coverage → PARTIAL/UNKNOWN, never empty | idx §0.1/§5/§7; admission order uses only canonical PageCursorV1/PageResult; locator selection binds exact trusted Principal; lens §7.2 | GV-14, GV-13 | read/result-registry matrix; WL-SPRAY; WL-DEAD-LOCATOR (dead/untrusted total visits + cursor) | CF-6 | COVERED |
...
| OR-C | Best-mirror ranking ON-CHAIN, zero new state beyond declared evidence (2026-07-15 C) | idx §7 `B0_SELECT` is the bounded on-chain single-score/latest selector scoped to one nonzero trusted Principal; untrusted evidence never wins. URI/content/health conformance and fallback are cont §10.3's `SELECT_PROFILE_V2`, client-tier/deferred | corrected GV-16 split suites | FX-ARC/FX-50GB B0 rows; CV-CLOCK; WL-DEAD-LOCATOR | CF-14 | COVERED |
...
| S-RD5 | R-D5 limits fail explicitly/deterministically; nothing silently truncates (:159) | [DERIVED INVARIANT] | enc §2.6/§2.7 typed errors; auth/realm guarded u48 Envelope inventory with exact `E_ENVELOPE_ORDINAL_EXHAUSTED()`; idx §5.2 (clamped maxItems never reverts, truncation labeled); every chapter's closed error table | GV-1, GV-8 boundary, GV-14 | CV-SPARSE-ADMIT; corpus MUST-FAIL members | CF-5 | COVERED |
...
| V2-E5 Realm bootstrap & authority history | RealmDescriptor/1 plus exact protocol 0.0 seven-field InitConfig/hash/GenesisFactsView/revision-1 policy+authority; canonical authority ref; direct/UUPS EIP-1967 facts and getters; enumerable paired revision/AuthorityTransition history and packed AdmissionBatch intent lanes; C-4 parses the state-readable AUTHORITY/INDEX Codex modules and module-completeness manifest while C-6 probes canonical admission pagination; C-1..C-7, QR-1..8, admission/finality split, U-1..6, guarded u48 Envelope inventory, and unsigned-carrier/receipt/lane-grounded reconstruction including receipt-authoritative W-7/W-4a | realm (all); enc §1.6; prin §3.8; idx §0.1; GV-17; CV-RECON; hf §3.2a |
