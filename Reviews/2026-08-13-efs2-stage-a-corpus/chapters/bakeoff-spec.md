...
- `RECON_1` — pass/fail: **both** arms complete the state-only reconstruction walk
  ...
  claims to recover or replay a discarded main witness. For effective T4, the
  accepted Withdrawal receipt/batch is the authoritative historical fact for
  main authority, target witness/evidence, Principal equality, policy, and code
  basis; retained evidence recomputes target identities without current/archive
  ERC-1271 execution or required witness replay. The u48 Envelope inventory
  loop also terminates safely at its maximum without wrap. [DERIVED INVARIANT as a hard gate —
...
| AuthorityVerifier/1 + AccountPrincipal/1 | b0-principal-authority.md §2–§3 | before any cell artifact mints, Stage B chooses one concrete `ERC1271_VERIFY_GAS` for the corpusVersion and every cell embeds the byte-identical principal-owned `authorityCodexBytes` as owner-module code 1: 30 constants, the closed `VerifierVM/1` 24-opcode semantics table, one ordered 11-step common prelude, four ordered profile programs (6/5/4/4 steps), and the 15-row error table; a cap or instruction/program variant requires a distinct corpusVersion and comparison run, never a cell-local profile; none except F2's AuthorRef entry |
...
| Generic page ABI, selector, and index Codex grammar | b0-indexes.md §0.1/§5.1–§7 | every cell embeds the byte-identical length-delimited `indexCodexBytes` as owner-module code 2 in the one Realm Codex hash and retains the `uint256` PageRequest/PageResult ABI and canonical `PageCursorV1` / `UniqueTypeCursorV1`: version/reserved bits; exact family-specific tags (ordinary query+mode, admission neither, unique Type+mode, all with Realm/realmBasis); separate `basisOrdinal` binding `H`; canonical end and physical/nested range validation; wrong context/basis or terminal input reverts `ErrPageCursor(uint256)`. Admission enumeration is only `admissionLogPage(PageRequest)`; `admissionAt` is point-only. The B0 selector requires one nonzero full-width trusted Principal and cursor-binds exact `realmId`, `realmBasisAt(H)`, target, and full SelectSpec; its separate `basisOrdinal` binds `H`, and initial/resumed calls recompute the Realm basis and tag. A wrong Realm or same-H/different-realmBasis cursor reverts `ErrSelectCursor(uint256)`. A zero initial basis resolves once; validated PARTIAL pages make deterministic physical progress and carry the exact continuation. No cell may replace these with a cell-specific or SDK-only grammar. |
| Content Type family + 50 GB client machine (ArtifactClosure STRING(255), structural/profile split) | b0-content-locators.md | none: B0 structural selection is trusted-Principal-scoped; URI/content/health validation and fallback stay client-side |
...
2. The fixtures/measurement lane publishes a **corpus manifest**: a content hash over
   ...
   profile. The manifest includes byte-exact `interfaces/index-codex.bin` and
   `profiles/codex-constants.bin`; the latter contains the former exactly once
   under its canonical u32 length. Every cell report embeds the manifest hash
   it ran against.
...
- **One aggregate snapshot per cell.** The mandatory bundle (baseline families +
  ...
  trusted-Principal-scoped best-locator view) is priced as ONE gas snapshot per cell against the corpus, never
