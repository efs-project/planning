...
| Generic page ABI and cursor grammar | b0-indexes.md §5.1–§5.2 | every cell retains the `uint256` PageRequest/PageResult ABI and canonical `PageCursorV1` / `UniqueTypeCursorV1`: version/reserved bits; exact family-specific tags (ordinary query+mode, admission neither, unique Type+mode, all with Realm/realmBasis); separate `basisOrdinal` binding `H`; canonical end and physical/nested range validation; wrong context/basis or terminal input reverts `ErrPageCursor(uint256)`. A zero initial basis resolves once; validated PARTIAL pages make deterministic physical progress and carry the exact ordinary or outer/inner continuation. No cell may replace it with a cell-specific or SDK-only cursor. |
