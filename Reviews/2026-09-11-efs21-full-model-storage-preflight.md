...
The [[2026-09-12-efs21-metadata-only-admission-plan|three-site task]] is now independently reviewed/root-reproduced/pushed at `1cb402a`. Whole create including staging falls 5,738,536 → 5,716,508 gas; referencing an existing 8,192-byte Record saves 566,522 gas. The existing nonzero-cache/no-code refusal remains; Record metadata checks retain malformed-header rejection and add an explicit 8,192-byte bound before avoiding payload copies. That new fail-fast handling is not falsely attributed to the old implementation. All-ACTIVE retry and representative public reads are unchanged controls. Persistent storage is unchanged, and later shared-byte comparisons must include these accessors in both arms.

### Large Types: compact before transport and storage

A further source/ABI preflight independently reconstructed the retained sixteen-Type pressure case: each Type has 64 BOOL fields with one-byte names. The group is 4,930 bytes and its length-prefixed Record body 4,932 bytes. Each raw ABI cache is 20,864 bytes, so **333,824 is the sum of caches**, not the complete helper response: the latter is **336,096 bytes**, over the unchanged 131,072-byte response cap. [Original exact framing](https://github.com/efs-project/planning/blob/1cb402a/Reviews/2026-09-05-c0-core/stateful-verification.md#next-resource-falsifier-small-group-large-opaque-cache). This is reference-parser/encoder and ABI evidence, not a new onchain helper execution or proof this whole group fits admission; the call-gas bound may fail earlier.

Splitting a single code blob does not solve this case: each member already fits the individual code limit. Even ignoring parsing, CREATE and all Core work, eagerly storing sixteen raw STOP-prefixed caches has a current code-deposit floor of `200 × (333,824 + 16) = 66,768,000` gas. Streaming output alone removes neither this cost nor the ordinary transaction ceiling. Splitting transactions distributes work, but does not erase their total bill or justify partial semantic Type visibility.

Reuse the existing [standalone lossless cache codec](https://github.com/efs-project/planning/blob/1cb402a/Reviews/2026-09-11-type-cache-codec-lab/README.md) before inventing another format. Its measured, non-integrated checkpoint has 19 actual-helper Types and two explicitly synthetic representation cases with independent JS/Solidity agreement. One BOOL cache is 704 → 121 bytes; a 64-field Type with long names is 24,960 → 5,536. Applying that documented layout to the sixteen-member short-name fixture gives **1,504 packed bytes per member / 24,064 aggregate** by arithmetic, not a measured admitted group. Exact canonical declaration bytes, Type IDs, descriptors, signed bounds and the logical ABI need not change.

The smallest promising integration compacts **inside the helper before its return is serialized**, retains compact code bytes, and decodes the exact logical cache at the consumers that need it. A physical format is not a new Type meaning. Format dispatch, canonical framing, checked header/TypeId/blob-hash reads, helper/codec identity, repeated group equality, Withdrawal and same-carriage dependency visibility remain load-bearing. The standalone codec's semantic-tag passthrough is not permission for invalid Types: the existing parser/admission still decides validity.

Module placement is still a design/size gate: the measured standalone codec is 7,700 runtime bytes and the current helper 18,953, so simply assuming both fit when inlined is unjustified. A read-only call-graph/pinning preflight is underway; no integrated saving or recovered 64-field admission is claimed. Compact storage can trade writes for read/decode work, so whole Type setup, small-Type controls, paid reads and complete Files operations must all be measured. Staged or lazy cache alternatives remain possible, with explicit abandoned-work cost, atomic visibility and deterministic recomputation requirements; a static contract read cannot persist a missing cache.

That independent call-graph preflight now recommends a separate stateless codec pinned by the helper's immutable address/hash, with bounded STATICCALLs and explicit transitive profile qualification. The 21-word ExecutionSet can remain the same shape while Core configuration commits to the new codec/profile; unchanged shape is not unchanged identity. Check actual codec code on use, not merely that the helper's runtime committed to an address. Packing cannot CREATE beneath `Preparation.invoke`'s STATICCALL; physical cache deployment remains a later ordinary helper call from the helper's account. Exact raw `TypeRow.cacheBytes`, group equality, active Withdrawal and checked Type headers all need explicit decoding paths. This is still an unimplemented integration recommendation.

Before integration, a separate [[2026-09-12-efs21-codec-scalar-write-plan|reviewed codec-only scalar-writer experiment]] targets its byte-at-a-time packing loop. A bounded backward-anchored masked store can preserve exact bytes without accessing memory beyond the logical end. The real writer, memory canaries, old/old performance RED and paid whole-codec comparison are required; no gas saving is inferred from source. The larger integration still must prove actual 64-field and sixteen-Type admission under ordinary caps, including the 35,168-byte unpack-response synthetic boundary rather than assuming the earlier synthetic case was the largest framing overhead.
...
**September12 Envelope-elision delta, source8688d52:** stored Envelope bytes are the **unsigned** header plus full ordered Record-ID vector; they contain no author signature. Native ingress removes separate AuthorIntent verification, not stored signature bytes. Admission rows do not themselves store RecordId: hydration, occurrence references, withdrawal and Binding predecessor checks recover it through that vector. Unselected vector members may have no Admission rows, so reconstructing only from admitted occurrences is insufficient.

A separate physical backend could retain write-once variable header fields/full vector, reconstruct fixed fields and ABI framing, and return the identical canonical Envelope bytes/identity through every existing read. It cannot return empty bytes or quietly require historical logs: EVM contracts cannot directly read old logs/transaction input. A witness-based alternative changes APIs and availability assumptions. Structured SSTOREs/reconstruction may cost more than the current immutable code blob, especially after a shared-byte block amortizes CREATE. Keep native authority and backend cost as distinct axes, retain all admission/lifecycle/index/Batch laws, and test a value+CAS publication, exact reconstruction, withdrawal/history/Lens and an unselected member before any parity claim. No backend or saving is implemented by this source review.
...
A literal adapter adds roughly four Store calls per append (head read, word read, word write, head write), plus first-key mirror maintenance; a live delta adds two. `occurrencePostings` also reads its leading family-3 head before the loop, so four is not the complete occurrence-maintenance count. This is source operation counting, **not a gas estimate**. It may initially cost more.

An independent read-only delta review at `1cb402a` supports a less mechanical first candidate: **separate storage plus atomic per-key maintenance**. The original in-Core append has no external call between mirror/word/head writes; moving that whole operation together preserves their indivisibility better than introducing separate external row setters. Without a literal-adapter intermediate arm, its cost result must be labelled separation **plus coalescing**, never attributed exclusively to separation.

The proposed Store surface is `append(key, admissionOrdinal, audit, stagedPostingKeyHigh) → (oldHead, newPostingKeyHigh)`, `liveDelta(key, increase) → oldHead`, and read-only `head`, `word`, `keyAt`. Core still derives keys/family policy and retains staged global counts; Store owns heads, words and mirror contents with only its immutable Core proxy authorized to write. No generic public row setter or mutable callback belongs in it.

Preserve append's actual ordering: ordinal/flags assertion, count/live guards, conditional new-key high-water/mirror, vacant-word assertion, word, then head. Pass the **current** staged mirror high-water: computing `next()` unconditionally in Core would wrongly reject an existing-key append at saturation. Preserve liveDelta's exact untouched bits and existing weaker admission checks rather than quietly importing all stronger read-validation predicates.

Return the pre-operation family-3 head from the first occurrence-key mutation. Keep family-2 maintenance **after the entire occurrence-key loop**, distinguishing first-ever occurrence (`count=0/live=0`), revival (`count>0/live=0`), and last-live withdrawal. This can remove the extra leading head read, but not the family-3 dependency. Raw Store reads can still expose provisional earlier keys while persisted Core counts are old; they are not qualified COMPLETE reads.

Before implementation, add a pinned deployment/configuration plan and exact source-frozen comparisons. Decisive tests are saturated new/existing mirror positions with compound bad flags/order/occupied-word faults; repeated keys and five-u48 rollover; last withdrawal/revival; later-key and later reference/CAS/cache rollback across both accounts; wrong Store owner/code/upgrade binding; and explicit raw-observer/reentry characterization. A literal adapter remains necessary if later claiming coalescing-only savings or its exact intermediate call-boundary behavior. No extraction or saving is implemented here.
...
[[2026-09-12-efs21-index-policy-boundary|The subsequent policy boundary]] distinguishes this physical extraction from James's intended transfer of configuration/key derivation. It preserves legacy Type-declared obligations while proposing an additive optional profile first, records that live multiplicity can belong to a mandatory coordinator rather than inherently to Core, and names the backfill/withdrawal counterexamples. No configurable full-C0 coordinator is implemented yet.
