...
- **Contract reads are cheaper too:** a reviewed fuller-model batch reduces eight distinct small Record reads from 453,212 to 283,890 gas in one real consumer transaction. It needs no preliminary context call. One-record batches cost more, and maximum-size bodies save only 1.1%; ordinary point reads remain useful. Real Files browse integration is next, not already proven.
- **Redundant storage removed:** reviewed metadata packing saves about 22,100 gas per fresh native Record. A fresh 41-byte raw edit is 241,339 gas; a retained contract quote update is 243,249. Reusing an existing Record costs 12 gas more. The next native storage experiment will select between bounded slot/code backends; tiny and zero-heavy values remain a weakness of always-code.
- **Now integrating real browsing:** the fuller-model Files reader is adopting explicit source-qualified anchor batches, with existing Lens/charter/occurrence checks retained. No directory reduction is claimed yet. No final feature sacrifice or protocol freeze has been decided for James.
...
That next [[2026-09-11-efs21-files-anchor-batch-plan|Files integration plan]] has an independently reviewed source seam and is now dispatched from reviewed`8f101f1`: explicitly capability-qualified batches hydrate only source-checked anchor Records. Occurrence, Binding, Lens and charter checks remain. Frozen scalar and candidate readers will browse the same deployed world/block; actual request counts, bytes and latency must improve without changing rows or completeness. No browse saving is measured yet.
...
The hybrid passed source preflight but is only a plan: fixed headerless words can avoid paying a second dynamic-bytes length slot, but zero-heavy reads still touch all words. Neither is an adopted layout. Preserve bounded corruption checks and exact read hashes on both paths, price selection overhead, whole-file writes, paid reads and helper setup, and do not treat the older post-copy-hash fixture as a prebounded corrupted-length control. Legacy code-only runners must use explicit frozen replay or refuse a hybrid selection, never silently call it always-code. Generic ingestion extraction remains a separate change so costs stay attributable.
