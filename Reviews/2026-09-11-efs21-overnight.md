...
- **Large-body storage is much cheaper, but not universally:** an independently reviewed native experiment, matching the returned-data hash check, reduces a fresh dense 4,032-byte edit from 3.08M to 1.13M gas. The new reader also has stronger pre-copy bounds; the old control does not match every corruption guarantee. Small and zero-heavy values can regress, so always-code is not the proposed blanket policy. The browser still runs the earlier stable source.
- **Contract reads are cheaper too:** a reviewed fuller-model batch reduces eight distinct small Record reads from 453,212 to 283,890 gas in one real consumer transaction. It needs no preliminary context call. One-record batches cost more, and maximum-size bodies save only 1.1%; ordinary point reads remain useful. Actual Files browse results below are a separate measurement.
...
- **Real browsing now measured, with a negative latency result:** source-qualified anchor batching reduces an eight-name/page8 listing from86 to79 browse-and-seal RPCs, but at normal defaults the50ms-delay median rises1.5%. Bytes increase too. This is a correct experimental integration, not a demonstrated speedup or a default-performance recommendation. No final feature sacrifice or protocol freeze has been decided for James.
...
The hybrid passed source preflight and is now dispatched from reviewed`03f0160` after the Files gate: fixed headerless words can avoid paying a second dynamic-bytes length slot, but zero-heavy reads still touch all words. No result or adopted layout yet. Preserve bounded corruption checks and exact read hashes on both paths, price selection overhead, whole-file writes, paid reads and helper setup, and do not treat the older post-copy-hash fixture as a prebounded corrupted-length control. Legacy code-only runners must use explicit frozen replay or refuse a hybrid selection, never silently call it always-code. Generic ingestion extraction remains a separate change so costs stay attributable. Its [[2026-09-11-efs21-native-kernel-extraction-plan|concrete implementation plan]] has passed independent read-only review but waits for the reviewed hybrid base: generic Record storage plus mandatory inventory, Files facade plus required Navigation/configurable Discovery, qualified as an actual dependency graph. Separation itself may cost more; measure it.
