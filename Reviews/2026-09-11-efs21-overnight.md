...
Measure always-code first against a frozen current storage control, including whole file creation/edit, tiny/large and zero/nonzero payloads, duplicate admissions, independent contract consumers and historical reads. Tiny bodies may lose after deployment/pointer overhead; choose a simple hybrid threshold only after evidence. No raised size limits, offchain substitution, or bare storage microbenchmark presented as whole-filesystem savings. The body backend can later move under generic record ingestion; combining that extraction with this first comparison would obscure causality. A three-arm implementation is underway, with a separate slot-backed read-integrity control so the new body rehash is priced fairly. No reviewed savings are claimed yet.
