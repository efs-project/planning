...
- **Now reducing read overhead:** a bounded current/checked Record batch is being implemented in the fuller-model branch. Contracts should qualify execution once per batch without needing a preliminary context call. No final feature sacrifice or protocol freeze has been decided for James.
...
The first implementation plan is now deliberately narrower: [[2026-09-11-efs21-checked-record-batch-plan|checked Record batching]], at most eight existing checked Record reads. A contract-facing current-state entry needs no preliminary context query; the SDK-facing checked entry adds exact expected-basis preconditions. Both validate the execution once and return basis with results. The existing 8192-byte per-record bound limits the largest return to 67,264 ABI bytes (about 134.6KB as a JSON-RPC hex response). History and Lens work are excluded from that cap, not silently bundled into it. Implementation is underway from reviewed `67f11f7`; no measured saving is claimed yet. Real Files directory integration follows its own measured gate.
...
The experiment order remains: preserve all behavior during physical extraction; test a counter alongside the old list as an oracle; then remove each redundant representation separately with an explicit replacement read/reconstruction contract. Counter changes must compose through the pending journal, especially multiple additions/withdrawals of the same record in one carriage. Required falsifiers include duplicate admissions, exact/mixed retries, last withdrawal and revival, a withdrawn first anchor with another occurrence still live, partial envelopes, ordinal aliases and atomic failure rollback.
