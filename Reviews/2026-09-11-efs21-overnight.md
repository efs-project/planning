...
- **Large-body storage is much cheaper, but not universally:** an independently reviewed, safety-matched native experiment reduces a fresh dense 4,032-byte edit from 3.08M to 1.13M gas. Small and zero-heavy values can regress, so always-code is not the proposed blanket policy. The existing browser still runs the earlier stable source.
- **Contract reads are cheaper too:** a reviewed fuller-model batch reduces eight distinct small Record reads from453,212 to283,890 gas in one real consumer transaction. It needs no preliminary context call. One-record batches cost more, and maximum-size bodies save only1.1%; ordinary point reads remain useful. Real Files browse integration is next, not already proven.
- **Now reducing redundant storage:** the native arm is packing explicit existence into an already-written metadata word, with the former mapping slot reserved. This is a same-semantics experiment, not an adopted layout or a claim that tiny values should use code storage. No final feature sacrifice or protocol freeze has been decided for James.
...
A read-only engineering review found a concentrated full-C0 extraction seam: store-row reads/applies plus two posting read primitives. That offers a later **same-semantics, separate-storage control**. It should preserve every family first and measure the extra call overhead before dropping mirrors/counters or changing coverage claims. Bytecode size and deployment/qualification changes are its early gates. The earlier journal/replay extraction preflight predates direct application and must be refreshed against that code, including provisional-state observation and cross-account rollback; it is not a ready unchanged implementation brief.
...
That next [[2026-09-11-efs21-files-anchor-batch-plan|Files integration plan]] now has an independently reviewed source seam: explicitly capability-qualified batches hydrate only source-checked anchor Records. Occurrence, Binding, Lens and charter checks remain. Frozen scalar and candidate readers will browse the same deployed world/block; actual request counts, bytes and latency must improve without changing rows or completeness. The plan is not dispatched or measured yet.
...
The large dense edit is **63.3% cheaper**, with identical action calldata. These are native-profile results, not an equivalent full-v2 create or a claim that a zero-body admission includes file placement. Setup increases by 420,269 gas over the safety-matched control and is priced separately. Tiny/zero-heavy regressions remain visible; all-zero slot words are cheap, whereas deploying bytes charges for zeros too. A length-only threshold is therefore not supported by these measurements.
...
The next storage questions are deliberately separate: first [[2026-09-11-efs21-packed-presence-plan|pack explicit presence with existing pointer/length metadata]], then evaluate a slot/code policy using nonzero storage-word occupancy as well as size. The packing plan passed independent read-only preflight and is now dispatched from reviewed`58e61c4`; no saving is claimed before its matched receipt gate. Neither change is an adopted layout. Sparse large bodies are missing from the current crossover sweep; the recorded mixed bodies still occupy every storage word. A hybrid must preserve bounded corruption checks and exact read hashes on both paths, and price selection overhead, whole-file writes, paid reads and helper setup. Generic ingestion extraction should remain a separate change so costs stay attributable.
