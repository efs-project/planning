...
- **Canonical Types now work through native Files and the SDK:** the [[2026-09-12-efs21-canonical-native-types-results|reviewed integration]] uses the actual structural interpreter, independent contract consumers, browser history and interrupted-write recovery. Root reproduced162native/58Node tests and193signed transactions. The matched quote-File edit costs228,331→295,135gas; stronger canonical validation adds66,804gas rather than reducing cost. Full admission, reference support and arbitrary developer validation are not implied.
...
**Current engineering gate:** the [[2026-09-12-efs21-body-copy-plan|bounded validation-copy experiment]] is complete at`ebc7d54`. The [[2026-09-12-efs21-canonical-native-types-results|canonical Types/Files integration]] is independently reviewed and root-reproduced at source/support`b8c2775`, evidence`d269e55`; it fits ordinary deployment limits. The [[2026-09-12-efs21-posting-store-plan|full-model mandatory index-contract extraction]] remains stopped at24,761bytes,185over the unchanged cap. Its next step is the reviewed typed read-facet split, then a same-topology index comparison. No paid index-saving or default/profile adoption follows from the native integration.

The index experiment uses one immutable Core-writer Store with explicit configuration binding. Its focused admitted-state ownership test went RED→GREEN, but the patch is incomplete and not deployable under the cap. It needs deliberate module decomposition, not removal of unrelated checks; this is not a falsifier of index separation itself. It is not yet the configurable-family redesign or a claimed gas win. A separate reviewed [[2026-09-12-efs21-live-files-plan|live-file adapter plan]] can use existing native Record/Files contracts unchanged; it remains staged, not implemented.
...
| Type language | Frozen control has three validator runtimes/native IDs; the new canonical bridge adds structural Types/constraints with reference/dependency/index-declaration refusal | Canonical structural Types/IDs, constraints and checked references, with known large-cache failures |
...
Not every native limitation is a required sacrifice. The new canonical bridge has now integrated a reference-free structural Type profile; its matched receipts price that added capability separately. Full-profile autonomous contract authorship is still an ingress gap in the compared prototype: even its direct authorized path currently requires an EOA signature. Neither compared arm includes arbitrary developer-programmed acceptance; separate designs/experiments are not automatically part of these prices. Account-native authority, reusable publication evidence and independent collaborative selections are meaningful profile choices, not synonyms.

**Current recommendation, not an owner ruling:** keep the richer publication model available, but do not assume every contract-owned value must pay for it. The canonical native bridge now prices structural typing: matched contract quote updates131,941→198,745gas, with unchanged native ownership/history/navigation promises. This is a different ordered workload from the earlier232,664 receipt, not a claimed saving against it. Next separate full-Core code placement from index storage costs, and price repeated schema-origin checks independently. Live files solve another problem—avoiding a duplicate app-state write—and do not eliminate the cost of genuinely authored stored content.

This interpretation was independently checked against the actual native producer transaction, full tag/edit runner and current authority entrypoints, not inferred from operation labels or slot counts. It changes no default or normative design; the stopped PostingStore patch remains separately preserved.
