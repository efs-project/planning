...
async function open(id,name){
  ...
  $('file-text').value=decoded??'';$('file-text').readOnly=decoded===null;$('save').hidden=decoded===null;$('convert').hidden=!kind;$('edit-representation').hidden=!kind;$('edit-representation').value='keep';$('rename').value=name;$('file-controls').hidden=false;$('history').replaceChildren();text('history-bytes','');
}
function chosenType(choice,current){if(choice==='keep'&&current)return current;if(choice==='canonical')return config.bytesType;if(choice==='raw'&&config.rawType)return config.rawType;throw Error('Unsupported representation choice');}
