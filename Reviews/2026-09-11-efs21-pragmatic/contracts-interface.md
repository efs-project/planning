...
The [current bounded hybrid experiment](evidence/hybrid-body.md) appends a private backend byte after that bool, preserving the two-word metadata and old roots, and adds a fixed128-word mapping at root6. New admissions validate, hash/dedup, check helper identity, then select code or words from exact length and masked nonzero-word occupancy. Duplicates never relocate. Code retains the same inert object checks; words require zero pointer, load only the bounded authoritative range, reject tail padding, and rehash the same identity. Empty word-backed records have explicit presence with no child or body word. There is no public selector/configuration API. Unknown tags and overlong lengths refuse; cleared presence and garbage beyond the authoritative range are not claimed detectable. The helper remains the same fourth constructor child and is checked even when words are chosen. Paid word reads can regress sharply despite cheaper writes. This is not a migration or a change to the running snapshotted browser.
