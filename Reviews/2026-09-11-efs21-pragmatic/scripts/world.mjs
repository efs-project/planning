...
export async function withWorld(action,{watchdogMs=300000,buildFirst=true,kernelArtifact='current'}={}) {
  ...
  return withManagedAnvil(['--host','127.0.0.1','--port',String(port),'--chain-id','31337','--hardfork','cancun','--gas-limit',String(GAS_LIMIT),'--accounts','0','--silent'],async node=>{
    ...
    async function rawSend(label,data,to) {
      ...
      setup.push({label,phase:'setup',hash,rawTransaction,gasUsed:BigInt(receipt.gasUsed).toString(),calldataBytes:(data.length-2)/2,calldataHash:E.keccak256(data),receipt});return receipt;
    }
    ...
    if(capabilities.bodyWriter){
      const helper=E.getCreateAddress({from:kernel,nonce:4});
      ...
      for(const refs of Object.values(deployed.immutableReferences))for(const ref of refs){
        ...
        expected=expected.slice(0,start)+E.zeroPadValue(kernel,32).slice(2)+expected.slice(start+64);
      }
