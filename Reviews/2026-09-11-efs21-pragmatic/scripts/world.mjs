...
export async function withWorld(action,{watchdogMs=300000,buildFirst=true}={}) {
  if (buildFirst) build();
  ...
  return withManagedAnvil(['--host','127.0.0.1','--port',String(port),'--chain-id','31337','--hardfork','cancun','--gas-limit',String(GAS_LIMIT),'--accounts','0','--silent'],async node=>{
    ...
    async function deploy(name,args=[]) {const a=artifact(name),i=new E.Interface(a.abi),data=a.bytecode.object+i.encodeDeploy(args).slice(2);assert((data.length-2)/2<=49152);return pin(name,(await rawSend('deploy '+name,data)).contractAddress);}
