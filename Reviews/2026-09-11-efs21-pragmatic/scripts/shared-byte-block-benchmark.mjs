// Full-C0 shared Record/Envelope blocks versus frozen independent Envelope/slot Record storage.
...
assert(['control', 'candidate'].includes(arm), 'usage: node scripts/shared-byte-block-benchmark.mjs control|candidate');
const ROOT = fileURLToPath(new URL('..', import.meta.url));
const VAULT = resolve(ROOT, '../..');
const preliminary=process.argv.includes('--preflight');
const output = preliminary ? join(VAULT,'.superpowers/sdd/2026-09-11-efs21-shared-slab-plan/preflight-'+arm+'.json.gz') : join(ROOT, 'evidence/shared-byte-block-' + arm + '.json.gz');
...
let managedCleanup;
const build = mkdtempSync(join(tmpdir(), 'efs21-shared-block-build-'));
process.env.EFS_TEST_BUILD_ROOT = build;
const CONTROL_COMMIT = '8688d5299eac8d9f83264806c32de471f427bbb2';
...
if(!preliminary) {
  const remaining=git(['status','--porcelain']).split('\n').filter(Boolean).filter(line=>!/^\?\? Reviews\/2026-09-11-efs21-pragmatic\/evidence\/shared-byte-block-(control|candidate)\.(json\.gz|manifest\.json)$/.test(line));
...
const supportPaths = ['scripts/shared-byte-block-benchmark.mjs', 'evidence/shared-block-control-sources.json', '../2026-09-09-files-browser-mvp/scripts/environment.mjs',
...
try {
  console.log('Compiling isolated shared Record/Envelope byte blocks foundation and Files router:', arm);
  ...
  const report = await withUpgrade(async lab => {
    ...
    async function cacheInventory(receipt) {
      ...
      for(let i=1;i<=envelopeCount;i++) {
        ...
        const row=(await receiptCall(receipt,lab.iface,'envelope',[id]))[0];
        {
          ...
          const extent=arm==='control'?length:Number((packed>>192n)&65535n),ordinal=packed>>(arm==='control'?192n:208n);
      ...
      for(let i=1;i<=Number(allCounts[0]);i++) {
        ...
        if(arm==='control') { records.push({id,words}); continue; }
      ...
      for(const [kind,rows]of [['Envelope',envelopes],['Record',records.filter(x=>x.address)]]) for(const x of rows) {
        const value=byAddress.get(x.address)??{kind:arm==='control'?'Envelope':'shared-block',slices:[]};
      ...
      if(arm==='control')assert.equal(children.filter(x=>x.kind==='Envelope').length,envelopeCount);
    ...
    if(arm==='candidate') {
      const packed=JSON.parse(readFileSync(join(ROOT,'evidence/shared-block-control-sources.json')));
      const compressed=Buffer.from(packed.gzipBase64,'base64');
      assert.equal(keccak256(compressed),packed.compressed.keccak256);
      const raw=gunzipSync(compressed,{maxOutputLength:4*1024**2});
      assert.equal(keccak256(raw),packed.canonical.keccak256);
      const frozen=JSON.parse(raw);assert.equal(frozen.commit,CONTROL_COMMIT);
      const oldCode=frozen.artifacts.PointReadLibrary.deployedBytecode.object;
      assert.deepEqual(frozen.artifacts.PointReadLibrary.deployedBytecode.linkReferences,{});
      assert(!frozen.artifacts.PointReadLibrary.deployedBytecode.immutableReferences);
      const expected=structuredClone(auth.expected);expected.components.PointReadLibrary.code=oldCode;
      ...
      historicalReaderRefusal={controlCommit:CONTROL_COMMIT,oldPointRuntimeHash:keccak256(oldCode),status:denied.status,reason:denied.reason,evidence:denied.evidence};
    }
    ...
    return plain({ arm, preliminary, generatedAt: new Date().toISOString(), sourceCommit: git(['rev-parse','HEAD']),
      ...
      storageProfile:arm==='control'?'envelope-code-admission-meta-initialization-v1':'shared-record-envelope-block-v1',soliditySourceCommit:arm==='control'?CONTROL_COMMIT:git(['rev-parse','HEAD']),
  ...
  const manifest={format:'EFS21_SHARED_BLOCK_RECEIPTS_GZIP_JSON_V1',canonical:{bytes:canonical.length,keccak256:keccak256(canonical)},compressed:{bytes:compressed.length,keccak256:keccak256(compressed)}};
