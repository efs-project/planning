// Full-C0 initialization outline / frozen inline initialization comparison. Same runner, exact isolated source selections; no tracing or server.
...
assert(['control', 'candidate'].includes(arm), 'usage: node scripts/initialization-outline-benchmark.mjs control|candidate');
const ROOT = fileURLToPath(new URL('..', import.meta.url));
const VAULT = resolve(ROOT, '../..');
const preliminary=process.argv.includes('--preflight');
const output = preliminary ? join(VAULT,'.superpowers/sdd/2026-09-12-efs21-initialization-outline-plan/preflight-'+arm+'.json.gz') : join(ROOT, 'evidence/initialization-outline-' + arm + '.json.gz');
...
let managedCleanup;
const build = mkdtempSync(join(tmpdir(), 'efs21-initialization-build-'));
process.env.EFS_TEST_BUILD_ROOT = build;
const CONTROL_COMMIT = '1cb402a19c9b6f1ddf4137dfa2597f85bc50dd82';
...
if(!preliminary) {
  const remaining=git(['status','--porcelain']).split('\n').filter(Boolean).filter(line=>!/^\?\? Reviews\/2026-09-11-efs21-pragmatic\/evidence\/initialization-outline-(control|candidate)\.(json\.gz|manifest\.json)$/.test(line));
...
const supportPaths = ['scripts/initialization-outline-benchmark.mjs', '../2026-09-09-files-browser-mvp/scripts/environment.mjs',
...
try {
  console.log('Compiling isolated initialization outline foundation and Files router:', arm);
  ...
  const report = await withUpgrade(async lab => {
    ...
    return plain({ arm, preliminary, generatedAt: new Date().toISOString(), sourceCommit: git(['rev-parse','HEAD']),
      sourceDiff: git(['diff','--','Reviews/2026-09-08-upgradeable-foundation/src/UpgradeAdmissionLibrary.sol','Reviews/2026-09-08-upgradeable-foundation/src/UpgradeableFixtureCore.sol']), supportPins, artifacts, contractSurface,
      ...
      storageProfile:'envelope-code-admission-meta-v1',soliditySourceCommit:arm==='control'?CONTROL_COMMIT:git(['rev-parse','HEAD']),
      bootstrapSnapshot, runtimes, operations, paidReads, paidConsumer, filesRead, externalHelperReceipt, cacheCases, inventory, inventoryBasis: { number: rejected.blockNumber, hash: rejected.blockHash }, transactions, cleanup: lab.cleanup });
  ...
  const manifest={format:'EFS21_INITIALIZATION_RECEIPTS_GZIP_JSON_V1',canonical:{bytes:canonical.length,keccak256:keccak256(canonical)},compressed:{bytes:compressed.length,keccak256:keccak256(compressed)}};
