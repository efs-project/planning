...
Post-review support-only change: the benchmark now explicitly rejects an incomplete inventory page before calling its source scan complete. A real 64/65-position boundary regression covers the guard. The retained 37-position measurements remain valid and were **not rewritten**; their support-source hashes describe the earlier measured script/test set, not this later guard and regression. Production contracts/artifacts are unchanged by this follow-up.
