...
contract LegacyPayloadConsumer {
    ...
    function read(LegacyNativeKernel kernel, address namespace, bytes[] calldata path)
        public
        view
        returns (bytes32 digest, uint256 length, uint64 revision, bytes32 recordId)
    {
        ...
        LegacyNativeKernel.FileInfo memory file = kernel.fileInfo(id);
    ...
    function capture(LegacyNativeKernel kernel, address namespace, bytes[] calldata path) external {
