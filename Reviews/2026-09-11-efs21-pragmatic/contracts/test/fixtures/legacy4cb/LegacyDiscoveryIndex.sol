...
import {LegacyExactTypeRegistry, LegacyUint256Validator} from "./LegacyExactTypeRegistry.sol";
import {LegacyNavigationIndex} from "./LegacyNavigationIndex.sol";

interface LegacyDiscoverySource {
    struct FileInfo {
        address owner;
        bool directory;
        bool live;
...
contract LegacyDiscoveryIndex {
    ...
    constructor(LegacyNavigationIndex nav, LegacyExactTypeRegistry registry) {
        kernel = msg.sender;
        navigation = nav;
        types = registry;
    }
    ...
    function attach(bytes32 exactType, bool required) external idle {
        ...
        LegacyExactTypeRegistry.TypeInfo memory info = types.typeInfo(exactType);
        if (info.codeHash != keccak256(type(LegacyUint256Validator).runtimeCode) || info.validator.codehash != info.codeHash)
        {
    ...
    function _maintain(address namespace, bytes32 id) internal virtual {
        Profile storage p = profiles[namespace];
        LegacyDiscoverySource.FileInfo memory f = LegacyDiscoverySource(kernel).fileInfo(id);
        bool eligible = f.owner == namespace && f.live && !f.directory;
        uint256 value;
        if (eligible) {
            LegacyDiscoverySource.Record memory r = LegacyDiscoverySource(kernel).readRecord(f.recordId);
