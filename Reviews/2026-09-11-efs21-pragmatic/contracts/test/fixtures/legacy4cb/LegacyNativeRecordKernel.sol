...
import {LegacyExpandedTypeRegistry} from "./LegacyExpandedTypeRegistry.sol";
import {LegacyBodyWriter} from "./LegacyBodyWriter.sol";
...
contract LegacyNativeRecordKernel {
    struct Record {
        bytes32 typeId;
        bytes body;
    }
    ...
    LegacyBodyWriter public immutable bodyWriter;
    ...
    constructor() {
        types = LegacyExactTypeRegistry(address(new LegacyExpandedTypeRegistry()));
        recordInventory = new LegacyRecordInventoryIndex();
        inventoryCodeHash = address(recordInventory).codehash;
        bodyWriter = new LegacyBodyWriter();
