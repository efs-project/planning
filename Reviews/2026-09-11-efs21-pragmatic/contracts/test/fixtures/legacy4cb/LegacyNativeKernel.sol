...
import {LegacyExactTypeRegistry} from "./LegacyExactTypeRegistry.sol";
import {LegacyNativeRecordKernel} from "./LegacyNativeRecordKernel.sol";
import {LegacyRecordInventoryIndex} from "./LegacyRecordInventoryIndex.sol";
import {LegacyNavigationIndex} from "./LegacyNavigationIndex.sol";
import {LegacyDiscoveryIndex} from "./LegacyDiscoveryIndex.sol";
...
contract LegacyNativeKernel {
    ...
    struct DirectoryPage {
        Entry[] entries;
        LegacyNavigationIndex.Cursor next;
        bool complete;
    }
    LegacyNavigationIndex public immutable navigation;
    LegacyExactTypeRegistry public immutable types;
    LegacyDiscoveryIndex public immutable discovery;
    bytes32 private immutable discoveryCodeHash;
    LegacyNativeRecordKernel public immutable recordKernel;
    ...
    function listDirectory(address owner, bytes32 parent, LegacyNavigationIndex.Cursor calldata cursor, uint256 limit)
        external
        view
        returns (DirectoryPage memory page)
    {
        _parent(parent, owner);
        LegacyNavigationIndex.Page memory ids = navigation.directoryPage(owner, parent, cursor, limit);
    ...
    function readRecord(bytes32 id) external view returns (Record memory) {
        _checkRecordKernel();
        LegacyNativeRecordKernel.Record memory value = recordKernel.readRecord(id);
