...
contract LegacyQuoteProducer {
    LegacyNativeKernel public immutable kernel;
    ...
    constructor(LegacyNativeKernel target, bytes32 exactType) {
        kernel = target;
        quoteType = exactType;
        operator = msg.sender;
...
contract LegacyQuoteReader {
    error MissingQuote();
    error UnexpectedType();

    function read(LegacyNativeKernel kernel, address publisher, bytes32 expectedType)
        external
        view
        returns (uint256 value, bytes32 fileId, uint64 revision)
    {
        ...
        LegacyNativeKernel.FileInfo memory file = kernel.fileInfo(fileId);
        if (!file.live || file.directory) revert MissingQuote();
        LegacyNativeKernel.Record memory record = kernel.readRecord(file.recordId);
...
contract LegacyPlainQuoteMapping {
