...
import {LegacyExactTypeRegistry, LegacyUint256Validator, LegacyBytesValidator} from "./LegacyExactTypeRegistry.sol";
import {LegacyRawBytesValidator} from "./LegacyRawBytesValidator.sol";
...
contract LegacyExpandedTypeRegistry {
    ...
    mapping(bytes32 => LegacyExactTypeRegistry.TypeInfo) private types;
    ...
    function register(bytes calldata descriptor, address validator) external returns (bytes32 id) {
        ...
        if (
            codeHash != keccak256(type(LegacyUint256Validator).runtimeCode)
                ...
                && codeHash != keccak256(type(LegacyRawBytesValidator).runtimeCode)
        ) revert UnsupportedValidator();
        id = keccak256(abi.encode(TYPE_DOMAIN, schemaHash, codeHash));
        if (types[id].validator == address(0)) {
            types[id] = LegacyExactTypeRegistry.TypeInfo(schemaHash, validator, codeHash);
    ...
    function validate(bytes32 id, bytes calldata body) external view {
        LegacyExactTypeRegistry.TypeInfo storage info = types[id];
    ...
    function typeInfo(bytes32 id) external view returns (LegacyExactTypeRegistry.TypeInfo memory info) {
        info = types[id];
        if (info.validator == address(0)) revert UnknownType();
    }
