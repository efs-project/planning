...
import {CanonicalTypeRegistry as ExactTypeRegistry} from "../src/CanonicalTypeRegistry.sol";
import {CanonicalFixtures} from "./fixtures/CanonicalFixtures.sol";
import {DiscoveryIndex, DiscoverySource} from "../src/DiscoveryIndex.sol";
import {NavigationIndex} from "../src/NavigationIndex.sol";

interface CanonicalFaultVM {
    function mockCallRevert(address, bytes calldata, bytes calldata) external;
    function mockCall(address, bytes calldata, bytes calldata) external;
    function clearMockedCalls() external;
}

contract CanonicalFaultDiscovery is DiscoveryIndex {
...
contract CanonicalDiscoveryFaultDriver {
    NativeKernel public immutable source;
    CanonicalFaultDiscovery public immutable index;
    bytes32 public file;
    bytes32 public exactType;

    constructor(NativeKernel k) {
        source = k;
        index = new CanonicalFaultDiscovery(k.navigation(), k.types());
    }
...
interface CanonicalDiscoveryAPI {
    struct Profile {
        bytes32 typeId;
        uint256 epoch;
        uint256 highWater;
...
contract CanonicalDiscoveryTest is TestBase {
    function canonicalKernel() internal returns (NativeKernel target) {
        ...
        return NativeKernel(deployed);
    }

    function registerDefaults(NativeKernel target) internal returns (bytes32) {
        (, bytes32[] memory ids) = target.types().registerGroup(CanonicalFixtures.group_defaults());
        return ids[0];
    }

    function registerBytes() internal returns (bytes32) {
        (, bytes32[] memory ids) = k.types().registerGroup(CanonicalFixtures.group_defaults());
        return ids[1];
    }

    function registerOtherUint() internal returns (bytes32) {
        bytes memory raw = CanonicalFixtures.group_defaults();
        raw[8] = 0x44;
        (, bytes32[] memory ids) = k.types().registerGroup(raw);
        return ids[0];
    }

    CanonicalFaultVM constant faults = CanonicalFaultVM(address(uint160(uint256(keccak256("hevm cheat code")))));
    NativeKernel k;
    CanonicalDiscoveryAPI d;
    bytes32 t;
    bytes32 root;
    ...
    function discovery() internal {
        ...
        d = CanonicalDiscoveryAPI(abi.decode(result, (address)));
    }
    ...
    function testLateAttachDuplicateZeroAndExactUniverse() public {
        ...
        discovery();
        d.attach(t, false);
        CanonicalDiscoveryAPI.Profile memory s = d.status(address(this));
        eq(s.highWater, 4);
        eq(s.through, 0);
        eq(s.health, 1);
        eq(d.probe(address(this), s.epoch, 0, a), 0); // UNKNOWN while building.
        CanonicalDiscoveryAPI.Page memory p =
            d.page(address(this), s.epoch, 0, CanonicalDiscoveryAPI.Cursor(0, 0, 0), 64);
        eq(p.ids.length, 0);
        yes(!p.complete);
        d.backfill(address(this), s.epoch, 0, 64);
        p = d.page(address(this), s.epoch, 0, CanonicalDiscoveryAPI.Cursor(0, 0, 0), 64);
        eq(p.ids.length, 2);
        yes(p.complete);
        eq(p.ids[0], a);
        eq(p.ids[1], b);
    ...
    function testEditsUnlinksAndCreatesInterleaveWithBackfill() public {
        ...
        CanonicalDiscoveryAPI.Page memory p = d.page(address(this), 1, 2, CanonicalDiscoveryAPI.Cursor(0, 0, 0), 64);
        eq(p.ids.length, 2);
        eq(p.ids[0], a);
        eq(p.ids[1], fresh);
        yes(p.complete);
        ...
        eq(d.page(address(this), 1, 2, CanonicalDiscoveryAPI.Cursor(0, 0, 0), 64).ids.length, 3);
        k.editFile(a, 3, t, abi.encode(uint256(3)));
        k.unlink(fresh, 1);
        eq(d.page(address(this), 1, 2, CanonicalDiscoveryAPI.Cursor(0, 0, 0), 64).ids.length, 1);
    ...
    function testStaleForeignAndEndCursorsAndLimits() public {
        ...
        CanonicalDiscoveryAPI.Cursor memory empty = CanonicalDiscoveryAPI.Cursor(0, 0, 0);
        CanonicalDiscoveryAPI.Page memory p = d.page(address(this), 1, 1, empty, 1);
        yes(p.complete);
        vm.expectRevert();
        ...
        vm.expectRevert();
        d.page(address(this), 1, 1, CanonicalDiscoveryAPI.Cursor(0, 1, 0), 1);
    ...
    function testToleratedChildFailureDirtyAndFreshEpochRecovery() public {
        ...
        vm.expectRevert();
        d.page(address(this), 1, 1, CanonicalDiscoveryAPI.Cursor(0, 0, 0), 64);
        vm.expectRevert();
    ...
    function testActualChildPartialWriteAndOOGAreIsolated() public {
        bytes32 a = make("a", 1);
        CanonicalFaultDiscovery fd = new CanonicalFaultDiscovery(k.navigation(), k.types());
        fd.attach(t, false);
    ...
    function testAuthorizationTypeValidationAndDetachedSemantics() public {
        ...
        vm.expectRevert();
        d.page(address(0xBEEF), 2, 1, CanonicalDiscoveryAPI.Cursor(0, 0, 0), 1);
        vm.expectRevert();
    ...
    function testAttachBeforeRootAndForeignQualifiedCursor() public {
        ...
        d.attach(t, false);
        d.backfill(address(this), 1, 0, 64);
        CanonicalDiscoveryAPI.Cursor memory cursor =
        d.page(address(this), 1, 0, CanonicalDiscoveryAPI.Cursor(0, 0, 0), 64).next;
        ...
        NativeKernel second = canonicalKernel();
        registerDefaults(second);
