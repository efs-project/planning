...
## Raw representation checkpoint

`1254c22` / evidence `a9a064e` adds an explicit raw-byte Type, not a new interpretation of old records. Matched fresh worlds use the same expanded registry/build and no optional discovery profile. A fresh 41-byte file costs **645,501 canonical / 598,309 raw gas**; its fresh-content edit costs **311,203 / 264,011**. The 47,192 saving is real, but that edit still misses the provisional 250k ambition. A uint256 producer update is unchanged at **245,563**. Raw 4096-byte admission versus canonical rejection is a capacity difference, not paired savings. [Complete receipts, setup, patterns, exact identities and limits](evidence/raw-representation.md).
