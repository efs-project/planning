...
const read=arm=>{
  const base=process.env.EFS_SHARED_BLOCK_PREFLIGHT_ROOT
    ? resolve(process.env.EFS_SHARED_BLOCK_PREFLIGHT_ROOT,`preflight-${arm}`)
    : new URL(`../evidence/shared-byte-block-${arm}`,import.meta.url).pathname;
  const manifest=JSON.parse(readFileSync(base+'.manifest.json')),compressed=readFileSync(base+'.json.gz');
  assert.equal(manifest.format,'EFS21_SHARED_BLOCK_RECEIPTS_GZIP_JSON_V1');
...
test('exact source selection, same support/compiler and unchanged helper runtime',()=>{
  assert.equal(control.soliditySourceCommit,'8688d5299eac8d9f83264806c32de471f427bbb2');
  assert.equal(control.storageProfile,'envelope-code-admission-meta-initialization-v1');assert.equal(candidate.storageProfile,'shared-record-envelope-block-v1');
  ...
  assert(changed.some(p=>p.endsWith('StateStore.sol')));assert(changed.some(p=>p.endsWith('StateKernel.sol')));
  ...
  if(!process.env.EFS_SHARED_BLOCK_PREFLIGHT_ROOT)for(const r of reports){assert.equal(r.preliminary,false);assert.equal(r.sourceDiff,'');}
  assert.deepEqual(candidate.operations.map(x=>[x.name,x.category,x.plan]),control.operations.map(x=>[x.name,x.category,x.plan]));
  if(!process.env.EFS_SHARED_BLOCK_PREFLIGHT_ROOT){
    ...
    const packed=JSON.parse(readFileSync(new URL('../evidence/shared-block-control-sources.json',import.meta.url)));
    const frozen=JSON.parse(gunzipSync(Buffer.from(packed.gzipBase64,'base64'),{maxOutputLength:4*1024**2}));
    const oldCode=frozen.artifacts.PointReadLibrary.deployedBytecode.object;
    assert.equal(candidate.historicalReaderRefusal.oldPointRuntimeHash,keccak256(oldCode),'exact source-backed old reader module refuses the new graph');
    const deployed=control.filesRead.evidence.find(x=>x.method==='eth_getCode'&&x.params[0].toLowerCase()===control.runtimes.PointReadLibrary.address.toLowerCase()).result;
    assert.equal(executable(deployed),executable(oldCode),'frozen router-root and matched foundation-root control Point reader differ only in compiler metadata');
    assert.notEqual(candidate.runtimes.PointReadLibrary.hash,control.runtimes.PointReadLibrary.hash);
  }
});

test('callable/event ABI stays exact; only physical Record and Envelope declarations change',()=>{
  ...
  delete fresh.RecordCell;fresh.EnvelopeCell=old.EnvelopeCell;
  fresh.Store=fresh.Store.map(x=>x.name==='records'?old.Store.find(y=>y.name==='records'):x);
  assert.deepEqual(fresh,old,'all other structures and mapping positions unchanged');
});
...
test('actual pointer/code inventory is chronological, with an explicitly unowned public helper child',()=>{
  for(const report of reports)for(const op of report.operations){
    ...
    for(const row of c.envelopes){const child=c.children.find(x=>x.address===row.address);assert.equal(child.kind,report.arm==='control'?'Envelope':'shared-block');assert(child.slices.some(x=>x.kind==='Envelope'&&x.id===row.id));assert.equal(row.offset,0);assert(row.length>=288&&row.length<=2304);assert.equal((child.code.length-2)/2,row.extent+1);assert(child.code.startsWith('0x00'));}
...
test('maximum selected workloads preserve ascending leaf order and retain bounded failures honestly',()=>{
  for(const report of reports)for(const name of ['64-unique-ascending-RecordIds','64-unique-reverse-RecordIds','64-duplicate-selected']){
    ...
    if(operation.category==='bounded-cap-rejection'){
      ...
      if(!process.env.EFS_SHARED_BLOCK_PREFLIGHT_ROOT)assert(operation.failedReplay?.message,'same-basis bounded refusal evidence');
...
test('partial reuse and ACTIVE retry allocate no Envelope; fresh existing-Records publication allocates exactly one',()=>{
  for(const report of reports){
    ...
    assert.equal(BigInt(mixed.cacheState.helperNonce)-BigInt(partial.cacheState.helperNonce),report.arm==='control'?0n:1n);
...
test('bootstrap remains exact; actual deployed modules retain ordinary headroom',()=>{
  ...
  assert.equal(control.runtimes.UpgradeableFixtureCoreU3.bytes,23619);
