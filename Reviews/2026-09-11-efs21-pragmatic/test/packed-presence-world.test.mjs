...
test('frozen always-code control and packed current preserve helper and exact identities',{timeout:180000},async()=>{
  const identities=[];
  for(const selection of ['baseline-58e61c4','current']){
    const result=await withWorld(async w=>{
      ...
      assert.equal(await c.rpc('eth_getCode',[E.getCreateAddress({from:helper,nonce}),'latest']),'0x00');
      ...
      assert.equal(BigInt(await c.rpc('eth_getStorageAt',[w.config.kernel,slot,'latest'])),selection==='current'?0n:1n);
