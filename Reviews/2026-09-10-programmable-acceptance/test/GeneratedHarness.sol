...
contract GeneratedHarness {
 ...
 function equip(EquipCodec.Fields memory fields,bytes32 codeHash,bytes32 outfitType) external pure returns(bytes memory,bytes32) { return(EquipCodec.encode(fields),EquipCodec.typeId(EquipCodec.rule(codeHash,outfitType),outfitType)); }
 function paid(PaidClaimCodec.Fields memory fields,bytes32 codeHash,uint256 fee) external pure returns(bytes memory,bytes32) { return(PaidClaimCodec.encode(fields),PaidClaimCodec.typeId(PaidClaimCodec.rule(codeHash,fee),fee)); }
 function checkOutfitRule(AT.Rule memory r) external pure returns(bytes32) {return OutfitCodec.typeId(r);}
 function registerOutfit(AcceptanceCore core,AT.Rule memory r) external returns(bytes32) {return OutfitCodec.register(core,r);}
 function checkEquipRule(AT.Rule memory r,bytes32 outfitType) external pure returns(bytes32) {return EquipCodec.typeId(r,outfitType);}
 function checkPaidRule(AT.Rule memory r,uint256 fee) external pure returns(bytes32) {return PaidClaimCodec.typeId(r,fee);}
}
