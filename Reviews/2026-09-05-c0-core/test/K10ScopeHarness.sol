...
contract K10ScopeHarness {
    ...
    constructor(StateKernel.Init memory init, address helper, bytes32 codehash, bytes32 libraryHash, uint256 mode) {
        ...
        StateKernel.selectScopeLayout(s, mode);
    ...
    function select(uint256 mode) external {
        StateKernel.selectScopeLayout(s, mode);
    }

    function scopeLayout() external view returns (uint256) {
        return s.scopeLayout;
    }
    ...
    function auditCounts(bytes32 scope) external view returns (uint64, uint64, uint64, bytes32, uint64) {
        return QueryReadLibrary.counts(s, 0, 10, 0, scope);
    }

    function corruptMode(uint256 mode) external {
        s.scopeLayout = mode;
    }

    function corruptKey(uint64 i, bytes32 key) external {
        s.bindingKeys[i] = key;
    }

    function corruptHead(bytes32 key, uint256 head) external {
        s.postings[key].head = head;
    }

    function corruptWord(bytes32 key, uint64 i, uint256 word) external {
        s.postingWords[key][i] = word;
    }

    function corruptBindingCount(uint64 count) external {
        s.count.bindingKeys = count;
    }

    function corruptAdmissionCount(uint64 count) external {
        s.count.admissions = count;
    }

    // Measurement consumer, not a production lookup/reverse-map API.
    function scopeLookup(bytes32 scope) public view returns (bytes32 key) {
        StateAuditPages.PageResult memory page =
            QueryReadLibrary.pagePostings(s, 0, 10, 0, scope, StateAuditPages.PageRequest(0, 1, 0));
        require(page.items.length == 1);
        uint64 physical = uint64(uint256(page.items[0]));
        if (s.scopeLayout == 1) return s.bindingKeys[physical];
        StatePointReads.HydratedOccurrence memory source = StatePointReads.hydrateOrdinal(s, physical, scope);
        bytes storage body = s.records[source.recordId].body;
        bytes32 purpose = bytes32(StorageByteView.word(body, 0, scope));
        bytes32 subject = bytes32(StorageByteView.word(body, 32, scope));
        bytes32 role = bytes32(StorageByteView.word(body, 64, scope));
        bytes32 position = keccak256(abi.encode(keccak256("efs2/position/1"), purpose, subject, role));
        return keccak256(abi.encode(keccak256("efs2/binding/1"), source.principalId, position));
    }

    function scopeHistory(bytes32 scope) external view returns (StateBindingReads.BindingHistoryEntry[] memory rows) {
        (rows,,) = QueryReadLibrary.readHistory(s, scopeLookup(scope), 1, 1);
    }
}
