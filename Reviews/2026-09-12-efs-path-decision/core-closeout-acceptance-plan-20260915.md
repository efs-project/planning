...
3. After all leaves, mandatory `afterPublication(publication,effects)` is a bounded STATICCALL. It validates final Name/Directory obligations, including Name-after-BIND. It does not append indexes or rerun Type rules. The full publication commits only if this returns successfully.
...
5. A publication lock covers all mutable ingress and mutable execution/index setters; proxy upgrade checks the same slot. Failure rolls back the lock. Normal next publication succeeds after a successful or reverted one.
...
- [ ] Use malicious callback fixtures to attempt nested publication, index replacement and proxy upgrade. Type rules remain STATICCALL and cannot mutate through descendants. Verify both prefix and final refusal restore complete state, not just return an expected error.
