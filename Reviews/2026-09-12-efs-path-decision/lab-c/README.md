...
## Sealed paid point/list slice (one A placement) — source only, no run claimed

The `typed-joined` cell of `script/measure.mjs` now mirrors the [[../sdk-fixture#Appendix — criteria for the next sealed paid point/list slice|SDK appendix]] in C's native representation: A1 (with the one `/swaps` `eth-usdc` placement), A2 (CAS), B1 (record + B's own HEAD, **no FOLDER bind**), then the exact post-B1 seal (snapshot id, block number/hash/timestamp, raw replies at that block proving the A placement's provenance and the absence of any B placement), then four paid rows (point/list × A-first/B-first) each the first and only transaction after an `evm_revert` to that seal at timestamp seal + 1 from a pinned unrelated caller (run mnemonic index 3), retained before the next revert with an `abstractResult` (the appendix's common comparison row, `inputEvidenceGrade: RPC_OBSERVED`, collapsing to `UNKNOWN` on any failed self-check). It refuses to run without `--anvil` before any chain call; the optional paired control `typed-joined/a1-without-placement` (A1 minus the FOLDER bind from the same pre-A1 state) is selected only by its exact `--cells` name. The A1 combined receipt is **not** a measured marginal placement cost (labelled ESTIMATE).

`test/MeasurementConsumer.sol` (test-only, stateless) carries the shared Quote → Pair → two Items closure for point and list at one pinned basis (exact Type ids, ordered refs, the selected Quote's fixture fields compared against inputs, selected revision, admission/Evidence range bounds and proof category, Realm/code commitments); the list additionally checks the one placement (COMPLETE window with an end condition, no duplicate/B placement, provenance = A1 / AUTHOR_A / EOA-signed effect, kept separate from content authorship); the point does no directory lookup. It decodes rows consumer-locally over the public `getRecord` ABI with pinned coordinates and emits one `PaidObserved` log with the concrete observations. `test/MeasurementFixture.sol` supplies the head-only B1; `test/FaultyReads.sol` is the armable forwarding reader for the malformed-reply controls; the broader `FixtureSeeder.b1()` (second placement) and its tests are unchanged. The expectation, arm-input and basis seals belong to the independent run controller, never to this packet (`MANIFEST.draft.json → sealedPaidSlice.operatorChecklist`).
...
| Ledger | `0xf698da25` `0x490423a3` `0xd5bfc2c4` `0x0134956e` `0x2986c0e5` `0xb09e8797` `0xc28e1de8` + `decodeTypeBody(bytes)` `indexCodehash()` | `domainSeparator() realmId() realmOrigin() coreCodeCommitment() index() highWater() rulesEpoch()` (`packetCommitment` removed: dead public code) | view/pure |
...
## EIP-170 decomposition (build #2d: Ledger 25,032 B, 456 over)

Coordinator option (1), ordinary and privilege-free: the shared write machinery is `src/ActionLib.sol` (INTERNAL library, inlined into the Ledger for `publishNative`/`publishSigned`); the import path is `src/ImportLib.sol`, an EXTERNAL library whose single function `importPublication` the Ledger reaches by the DELEGATECALL the compiler emits for external library calls. Consequences: storage stays in the Ledger; `msg.sender`/`address(this)` are the caller's/Ledger's; events are emitted from the Ledger; the library has no storage and no privileged function; its address is linked into the Ledger's bytecode at deployment (immutable) and its address + codehash are part of the deployment identity (`MANIFEST.draft.json → build.linkedLibraries`; `script/measure.mjs` deploys it first and links the placeholder). Errors and events moved to file level (`src/LedgerErrors.sol`) so every path raises the same selectors. Dead code removed: the public `packetCommitment` view (now internal to ImportLib; tests/script compute it locally). Cost line to charge: one DELEGATECALL per import (ESTIMATED ≈ 2,600 gas cold + calldata copy of the packet); publish paths are unchanged. ESTIMATED sizes after the split (unmeasured): Ledger ≈ 18–20 KB, ImportLib ≈ 15–18 KB, IndexModule/LensReader unchanged. If the Ledger is still over, next step is moving `ActionLib.publish` behind the same external library (a second DELEGATECALL per publication, also to be charged); reducing the `StoreRead` surface stays the last resort (reader impact noted in TODO).

**Test layout (EIP-3860):** the test contracts' initcode exceeded 49,152 B when every builder/decoder was inlined, so `test/Fixture.sol` is a separately deployed helper (realm deployment, ids, builders, signing, seeding, export) reached through thin wrappers in `test/LabBase.sol`; `test/DenialProbe.sol` holds the raw-write probes; the fixture suite is split into `test/Publish.t.sol` (steps 1–4, rejections, authorship closure), `test/Selection.t.sol` (steps 5–6, cursor/dedupe/coverage, paid consumer) and `test/Import.t.sol` (self-check reconstruction, import, deployment-bound identity, spoof-via-import).
