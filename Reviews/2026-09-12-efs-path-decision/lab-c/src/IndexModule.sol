 ...
 *
 * Authority: `attach(ledger)` is a one-shot call by the deployer (needed because
 * Ledger and IndexModule reference each other); after it there is no privileged
 * function. `poisonConcept` is an immutable TEST LEVER: a tag binding to that
 * concept reverts inside the index so the late-index rollback path can be forced.
 */
...
contract IndexModule is EfsStoreCore, IIndexModule {
  ...
  function onPublication(bytes32, uint64, Effect[] calldata effects) external {
    if (msg.sender != ledger) revert NotLedger(msg.sender);
    for (uint256 i = 0; i < effects.length; i++) {
      ...
      if (e.kind == KIND_RECORD || e.kind == KIND_DECLARE_TYPE) {
        ...
        if (e.purpose == PURPOSE_TAG && e.role == poisonConcept) revert IndexPoisoned(e.role);
  ...
  function coverage(bytes32 family, bytes32) external view returns (uint8 status, uint64 through) {
    ...
    if (!declared) return (COV_UNKNOWN, 0);
