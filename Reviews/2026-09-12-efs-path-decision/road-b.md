...
## 10. v1.3 — the paid point/list slice (lab `2859147`, compiled and tested; inputs `b2aa07d`; UNMEASURED; 2026-09-13 10:29 UTC)

Road B's counterpart of the pre-sealed A1/A2/B1 point/list comparison ([[sdk-fixture#Appendix — criteria for the next sealed paid point/list slice|appendix]]; [[matched-cost-scope-review]]) is source-complete and compiled: the Core is unchanged at `ca1a228`; the measurement consumer gained paid point and list entry points that observe placement provenance (the one A placement: A1, AUTHOR_A, EOA-signed effect) separately from selected-content authorship, share the Quote → Pair → two Items closure at one pinned admission frontier, compare the sealed value, `observedAt` and note commitment against inputs, check revision, a live BIND admission inside its publication range and the proof category (native publications carry zero signature words; signed ones do not), and, for the list, require a COMPLETE, ended, one-row window over exactly one raw candidate so a second placement or a duplicate fails. The runner seals the post-B1 block and snapshot, proves the absence of any B placement from raw state reads, runs each of the four paid rows first-from-revert at the same block number, parent and timestamp with an unrelated pinned caller, and retains the appendix's common comparison fields as `RPC_OBSERVED` observations that collapse to UNKNOWN whenever the candidate self-check fails. Six controls feed the consumer corrupted or missing actual replies and assert the specific refusal; they are named apart from the wrong-expectation negatives. Root's source review found no ABI blocker and its four comparison fixes are applied; the pin passed 52 Forge tests in the 10:10 UTC slot. B's fixture map, a self-verifying script (63 derivable values recomputed offline) and the seven-row expectation mapping are candidate-authored inputs for the run controller, which re-derives them independently. No paid row has been measured: the sealed `--anvil` run needs the controller's expectation, arm-input and basis seals and its own lease, and the once-only placement cost stays an ESTIMATE unless the paired control cell is pinned.
