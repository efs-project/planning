...
- Next handoff: C repair `774dfcd` and B public profile `885d9f9` passed independent review/root reruns; code branches are pushed at `7876477` / `2824297`. [[road-c-review|C's measurement preflight]] is complete and its script/test-consumer implementation is queued, not yet leased. Keep the typed Pair/Quote journey distinct from a supplemental 32-byte-value diagnostic under C's larger framing. SDK's `RPC_OBSERVED` extension returned `0b9fbaf`: root reran 85/85 tests and its byte-identical report, but publication is held for a confirmed malformed-as-omitted input bug. SDK PM owns a **20-minute, regression-first repair beginning 03:09 UTC** in its existing `lab-oracle/` branch only; no main, push or heavy run. [[oracle-implementation-plan#RPC_OBSERVED handoff intake — September 13, 03:09 UTC|The intake]] records exact reproductions, unchanged strict files and remaining unknowns. After that fix, review/publish and extend only from the reviewed public B profile/vector, never candidate helper/verifier code. No new architecture, feature waiver or James decision is needed.
