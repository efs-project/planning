...
Run `script/core-index-materialization.mjs finite` (or omit the argument, which now defaults to `finite`) with existing `FOUNDRY_OUT`, `ANVIL_BIN`, and `EFS_ETHERS_PATH` variables. Build matching current source into `FOUNDRY_OUT` first: the runner records the supplied source/artifact identities, not a reconstruction of historical artifacts. It closes only its owned disposable node and writes only `finite-paid.json.gz`.

Historical `old350k` mode and unknown modes are rejected before the resource module is imported, so they cannot start a node, deploy artifacts, or overwrite historical receipts. The original350k failure is retained historical evidence, **not reproducible by switching a label against current artifacts**. Reproducing it would require the exact historical source/artifacts and a separately verified historical runner. The fixed runner's source hash necessarily differs from the source snapshot embedded in the retained paid runs; neither paid archive was regenerated for this CLI-only correction.

Non-chain regression: `node --test script/core-index-materialization.test.mjs` exercises the actual entry point, intercepting only the external environment boundary. RED had3 failures (default selected the old fixture; historical and unknown modes did not reject before resource import), then GREEN4/4. Default/explicit finite select the full finite fixture; historical/unknown reject before resources; historical archive bytes are checked unchanged. No chain or Solidity behavior was changed for this correction.

`script/retain-core-index-evidence.mjs` losslessly packs named original logs, checks all retained receipt gas/status fields and code ceilings, and emits hashes. `evidence-manifest.json` identifies compressed/raw hashes and sizes. Exact test commands and detailed task report are in the local SDD report.
