...
function audit(packet){
  ...
  for(const t of transactions){
    ...
    assert.equal(tx.data,t.data);assert.equal(lower(tx.to),lower(t.to));assert.equal(BigInt(tx.nonce),BigInt(t.nonce));
...
result.mutationChecks=['unbound setup transaction metadata rejected','wrong raw return rejected','wrong receipt log rejected','wrong displayed gas rejected'];
writeFileSync(`${run}/audit2.json`,`${JSON.stringify(result,null,2)}\n`,{flag:'wx'});
