...
const result = {phase:'GREEN2',baseCommit:base,startedAt:new Date().toISOString(),compilerSha256:sha(readFileSync(solc)),dirtyDiffSha256:sha(git(['diff','HEAD','--binary'])),sourceHashes:sourceHashes(),stages:[],artifactHashes:{}};
assert.equal(result.compilerSha256,'738dcdc6afddeb505ee4e4ef24f1c1fdba2b8c924e614cbbf5801a5b062dd683');
writeFileSync(`${run}/green2.diff`,git(['diff','HEAD','--binary']),{flag:'wx'});
const save=()=>writeFileSync(`${run}/green2.json`,`${JSON.stringify(result,null,2)}\n`);
...
try{
  await invoke('green2-focused','forge',['test',...common,'--match-contract','JoinedConsumerTest','--match-test',matchTest,'-vvv']);
  await invoke('green2-full','forge',['test',...common]);
  await invoke('green2-sizes','forge',['build',...common,'--sizes']);
  await invoke('green2-node',process.execPath,['--test',...readdirSync(`${lab}/script`).filter(n=>n.endsWith('.test.mjs')).sort().map(n=>`script/${n}`)]);
