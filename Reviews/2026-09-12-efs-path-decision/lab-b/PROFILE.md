...
## Changed after `dcc7b94` — authority repair (2026-09-13; COMPILED AND TESTED at `ca1a228`: 43/43, no via-IR stack error; six-cell receipt diagnostic retained as `evidence/measure3-ca1a228.json`, 2026-09-13 09:00 UTC))

**Two qualifications carried forward from the coordinator's 08:56 handoff (both reconstruction gaps, not acceptance bypasses):**

1. `acceptanceBasis(ordinal).epoch` is the **policy activation-row epoch** (the registry epoch at which that row was appended), whereas the signed acceptance profile folds the **admission-context global registry epoch** (`registry.epoch()` at signing). The two coincide only when no registration or activation happened between the row's activation and the admission; an independent checker must join the basis row to the registry's activation rows by `(typeId, row)` and separately recompute the profile from the global epoch retained in the packet — it must not read the basis epoch as the profile epoch.
2. The lab `Reconstructor` **reuses the retained acceptance-profile hash** from the evidence cell; it does not regenerate that profile from its dependencies (Type id, mandatory ruleId, active policy codehash, global epoch). Reconstruction of the profile is therefore a candidate self-check only until an independent implementation recomputes it from the retained registry replies. Mutable mandatory validators remain an explicit dependency-semantics limitation (a codehash pins code, not dependencies); nothing extracted from `measure3-ca1a228.json` is an independently pre-sealed expectation.
