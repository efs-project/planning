...
node script/measure.mjs --anvil --out "$FOUNDRY_OUT/../measure.json"   # spawns a finite-history Cancun Anvil on a free loopback port, deploys, runs, kills it
```

`script/measure.mjs` needs ethers v6: set `EFS_ETHERS_PATH` to a `node_modules/ethers` directory (it looks in `planning/Reviews/2026-09-04-mvp-rehearsal/node_modules/ethers` and the `planning-efs21` sibling worktree by default). It asserts the four exact payload controls' Keccak-256 values from `run-manifest.md` before doing anything. Options: `--rpc URL --deploy`, `--rpc URL --addresses out/lab-addresses.json`, `--skip-without-index`, `--mnemonic`.

What the script prints: per cell of the authorship × selection matrix (`neither`, `authorship`, `selection`, `both`) and per fixture (`quote` 32 B, `binary` 41 B): `create` (subject + record + head + placement), `edit` (fresh body + CAS head rebind), `create-competing` (second author), paid consumer `read-resolve`, `read-resolve-second-first`, `read-list`, `read-history-asof`, an `eth_call` estimate for the browser row; then the freshness controls (`contract-fresh-body` with a proved pre-absence, `contract-existing-body`, exact-operation retry), the failure rows (wrong-Type/missing reference, stale CAS, failed acceptance), the without-index-module pass, and `interaction = both − authorship − selection + neither` per operation. Fresh-slot counts are not traced; the JSON carries the design's estimates, labelled.
