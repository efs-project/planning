...
contract FilesCarrierIndex is FilesDirectoryIndex {
    ...
    function onAdmission(uint64 publication,Effect[] calldata effects) public override {
        ...
        for(uint256 i;i<effects.length;i++){
            Effect calldata e=effects[i];if((e.kind!=1&&e.kind!=2)||e.typeId!=carrierTypes[3])continue;
            ...
            if(first==e.admission)_append(Keys.referenceList(t,0,FilesLayout.word(Ledger(ledger),e.recordId,0)),first,true);
        }
    }
}
