...
- **`script/measure.mjs` is NOT patched** (per instruction). It still passes `T.*` name hashes as Type ids (line 117; 48 uses) and its `register` calls (lines 1015–1022) succeed but return derived ids the script ignores, so every later action would hit `E_UNKNOWN_TYPE`. Runner patch owed before any receipt follow-up: after each `register` send, set `T.X` from an `eth_call` of `TypeRegistry.typeIdOf(shape, acceptor, refs)` (or the `register` return) and deploy `JoinedConsumer` with the derived ids (line 1012 must move after registration). The two new receipt rows (policy activation, refused re-registration) are described in `MANIFEST.draft.json` but not scripted for the same reason.
