# Controller interface — opt-in pinned run controller for `script/measure.mjs` (DESIGN ONLY)

**Status: proposal, no runner code yet.** Root owns the controller module and the independently derived vectors; this lab owns only the runner integration, its tests and docs. Candidate-authored; nothing here seals or repairs any completed run. Worked example: `vectors/controller-interface.example.json` (one message of each kind; build-dependent values are labelled placeholders).
...
| `--expectations <path>:<sha256>` | Root's neutral-expectation file (labels only) | operator-pinned | same sha256 check; the runner never opens or parses it — it only pins and echoes the hash |
| `--arm-input <path>:<sha256>` | Root's B arm-input manifest | operator-pinned | same; never parsed by the runner |
...
All three flags are required together; any one alone ⇒ `CONTROLLER_PINS_INCOMPLETE`, exit 2, before Anvil starts. Without the flags the run is **ungated**: `report.gating = "diagnostic"`, and `report.controller = null`. With them: `report.gating = "controller-gated"`. The three sealing cells (`joined/paid-slice`, `joined/a1-without-placement`, and any first selected cell for `beforeFixture`) keep their existing `--anvil`-only rule; the concrete `PaidResult` log and the cells themselves are unchanged.
...
- `afterB1`: inside `joined/paid-slice`, immediately after `sealState` and the basis reads (`counts`, `generation`, `epoch`, `coreCodeCommitment`, `realmId` at the sealed block) and BEFORE the placement replies (`seal-placement`, `seal-no-b-placement`) and the paid rows. `joined/a1-without-placement` has no `afterB1` (no B1).
...
| `chain.{chainId, source, anvilArgv}` | number, string, string[] | runner-observed | informational for the controller's arm-input check |
| `source.{commit, dirtyDiffSha256, sourceHashes}` | string, string, {path: sha256} | runner-observed | controller compares against the arm-input file; runner does not |
| `build.{solc, viaIR, optimizerRuns, evm, artifacts.<C>.{artifactSha256, initcodeHash, runtimeCodehash, runtimeBytes}}` | mixed | runner-observed (this run's build) | controller compares against its own build; runner does not |
...
| `inputs.ordinals.{placementAdmission, placementPublication, aHeadAdmission, aHeadRevision, bHeadAdmission, bHeadRevision, postB1Frontier, registryEpoch, indexGeneration}` | decimal strings | controller-supplied | compared against the runner's expected-under-this-runner constants; `postB1Frontier` must equal both `expect.*.basisAdmission` |
...
| `setupReceipts.{step1,A1,A2,B1}.{txHash, block, status, gasUsed, publication}` (+ `B1.actions`, `B1.folderBind=false`) | mixed | runner-observed | informational for the controller; `B1.folderBind` must be `false` |
| `observedOrdinals.{placementAdmission, placementPublication, placementRevision, aHeadAdmission, aHeadRevision, bHeadAdmission, bHeadRevision}` | decimal strings | runner-observed (raw replies at the sealed block) | must equal `inputs.ordinals.*` (`CONTROLLER_INPUT_MISMATCH:ordinals.<f>`) |
| `noBPlacement.{bHeadStateAtPlacement, bSwapsScopeCount, bOnlyLensStatus}` | decimal strings | runner-observed | all must be `"0"` |
