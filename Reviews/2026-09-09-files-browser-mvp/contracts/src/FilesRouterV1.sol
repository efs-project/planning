...
// Disposable prototype FilesRouter for the local files-browser MVP.
// Enforces per-author authority, the ASCII name profile and plan-wide
// source/destination preconditions ON-CHAIN, then executes the exact
// multi-leaf publication atomically through the existing Core.
// HONEST LIMITS: the Core admission is still authorized by the synthetic
// operator FixturePlan signature, which is bearer evidence — a holder could
// submit it directly to Core, bypassing this router. Operations executed
// here are therefore router-checked but NOT FILES_PRECONDITION_CERTIFIED
// (RoutedAdmissionIntent consent binding remains future Core work).
pragma solidity 0.8.30;

interface IFixtureCore {
...
contract FilesRouterV1 {
    ...
    IFixtureCore public immutable core;
    ...
    bytes32 private constant ACTION_TYPEHASH = keccak256(
        "FilesAction(uint8 kind,bytes32 mountId,bytes32 parent,bytes32 nameHash,bytes32 sourceParent,bytes32 sourceNameHash,bytes32 object,bytes32 aux,bytes32 publicationHash,uint64 nonce,uint64 deadline)"
    );

    mapping(bytes32 => address) public authorAccount; // principal -> one-time claimed signer
    mapping(bytes32 => uint64) public authorNonce;
    ...
    constructor(IFixtureCore core_, TypeIds memory t, Purposes memory p) {
        core = core_;
        typeIds = t;
        purposes = p;
    ...
    // One-time first-claim binding of a fixture principal to a local signer.
    function claimPrincipal(bytes32 principal) external {
        if (authorAccount[principal] != address(0)) revert ErrPrincipalClaimed(principal);
        authorAccount[principal] = msg.sender;
    ...
    {
        IFixtureCore.ResolveResult memory r =
    ...
    function _authorize(
        FilesOp calldata op,
        IFixtureCore.Publication calldata publication,
        uint64 nonce,
        uint64 deadline,
        bytes calldata authorSig
    ) internal {
        if (deadline == 0 || block.timestamp > deadline) revert ErrExpired(deadline);
        bytes32 principal = publication.header.principalId;
        if (authorNonce[principal] != nonce) revert ErrAuthorNonce(principal, authorNonce[principal], nonce);
        ...
        require(authorSig.length == 65, "sig");
        bytes32 r;
        bytes32 s;
        uint8 v;
        assembly ("memory-safe") {
            r := calldataload(authorSig.offset)
            s := calldataload(add(authorSig.offset, 32))
            v := byte(0, calldataload(add(authorSig.offset, 64)))
        }
        address recovered = ecrecover(digest, v, r, s);
        address expected = authorAccount[principal];
        if (recovered == address(0) || expected == address(0) || recovered != expected) {
            revert ErrUnauthorizedAuthor(principal, recovered);
        }
        authorNonce[principal] = nonce + 1;
    }

    // ---- publication template checks ---------------------------------------
    function _leaf(IFixtureCore.Publication calldata p, uint256 i, bytes32 expectType)
        private
        view
        returns (bytes memory body, bytes32 id)
    {
        if (p.leaves.length <= i) revert ErrTemplate(uint8(i), 1);
        IFixtureCore.SelectedLeaf calldata l = p.leaves[i];
    ...
    function execute(
        ...
        uint64 authorNonce_,
        uint64 authorDeadline,
        bytes calldata authorSig
    ) external returns (IFixtureCore.AdmitResult memory result) {
        _authorize(op, publication, authorNonce_, authorDeadline, authorSig);
        ...
        result = core.executeFixture(publication, expectedRevision, coreNonce, coreDeadline, operatorSig);
