...
> **Base native-AA update (reviewed 2026-07-19):** EIP-8130 adds a native transaction path that can batch calls directly from the configured sender and attach a separate payer, preserving the EAS `msg.sender`/attester invariant without an EntryPoint, bundler, or EFS gateway. Keep EIP-5792 `wallet_sendCalls` as the preferred wallet-facing capability API—supporting wallets may route it over native AA—and add a raw EIP-8130 mechanism only after the Vibenet matrix in [[Reviews/2026-07-19-base-native-aa-impact]] pins a tested spec/reference revision. Do not freeze the current `WriteMechanism` literal or receipt atomicity assumptions around a Draft EIP: EIP-8130 has call phases whose earlier phases can persist when a later phase fails, so an EFS all-or-nothing batch must use one phase and receipt handling must inspect per-phase status. This further weakens the case for building `EFSUploadGateway`; re-evaluate it after the spike.
