...
**2. Two patterns: private data vs. shared config — and namespacing is a navigation nicety, *not* a collision-avoidance mechanism.** Devs arrive expecting Unix, where you avoid conflicts by separating paths. In EFS you don't need to: attester-keying already isolates writes, so two apps writing the same path never clobber. So the path you choose depends only on whether the data is *yours* or *shared*:

- **Private data** (one app's own state) → namespace under a prefix you control, **`/apps/<reverse-dns>/…`** (e.g. `/apps/com.aave.v3/risk-params`) or address-rooted `/0x…/`. This is purely for human navigation and squatting-resistance — it is *not* required for correctness. Anchors are permanent (ADR-0030), so the path tree is append-only shared structure; the convention keeps it legible.
- **Shared config** (the common, interesting case) → a **deliberately shared, well-known path** like `/swaps/maxSlippage` that many apps read together. Here you do **not** namespace per-app — the whole point is a common path. Disambiguation is by **lens, not by path**: the reader chooses *whose value to trust.*
...
**2026-05-28 (entry 2 of 8) — Q1–Q5 resolved (James), folded in one pass.** Q1 (single `sdk/` repo) and Q2 (resource-oriented namespaces) confirmed as already designed. Three design changes (*Q1, Q3 and the Q5 mechanism order below were later superseded — see entries 3 and 4*):
...
**2026-05-28 (entry 4 of 8) — on-chain/off-chain reframe (James clarification).** James clarified two framing errors carried since the PM brief: (1) the **on-chain SDK is a Solidity deliverable** — a *library* (+ inheritable base) that smart-contract devs use *from their own contracts*, not a TypeScript package; and (2) **"off-chain SDK" just means "the TypeScript SDK"** — it has nothing to do with The Graph / a packaged indexer. Changes folded in:
...
**2026-05-28 (entry 5 of 8) — expert review + brainstorm on the reframe (2 agents: Solidity/EAS fidelity, SDK coherence + on-chain ergonomics).** The fidelity agent confirmed the attester thesis is **correct** (an `internal` library inlines into the consuming contract; even a delegatecall-linked library preserves `msg.sender` — only a plain `CALL` to a separate deployed helper breaks it) and the Solidity signatures are sound. Required fixes (folded in):
