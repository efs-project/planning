...
**2. Namespace under a prefix you control.** Because anchors are permanent (ADR-0030), the path tree is append-only shared structure anyone can extend. Recommend a convention so it stays coherent and squatting is moot: root each app under **`/apps/<reverse-dns>/…`** (e.g. `/apps/com.aave.v3/risk-params`) or an address-rooted `/0x…/`. This is prose guidance, not an enforced rule — EFS has no central path authority by design.

**3. Reading another team's config.** Pass their address as the lens: `read("/apps/com.aave.v3/risk-params", [aaveAddr])`. You must *know* `aaveAddr` — from a registry contract, a well-known constant, or out-of-band. `containsAttestations(target, aaveAddr)` checks O(1) whether they've written before you read. This is the common, fully-supported case.

**4. Canonical / shared config (the `/etc` problem) — partly solved, partly an open protocol call.** For unrelated teams to *agree* on shared config (a token registry everyone reads), someone's overlay must be the agreed-canonical one — and lenses are per-reader, so there is no automatic global truth. Off-chain, ADR-0039 already defines a default chain with a `systemLenses[]` tail tier (the deployer, read at runtime from `EFSIndexer.DEPLOYER`) and a reserved web-of-trust tier. **On-chain there is no automatic chain** — a contract must pass lenses explicitly — so the on-chain `/etc` answer today is: read canonical config through an agreed system/curator lens, e.g. `[EFSRouter.DEPLOYER()]` or a registry-published curator address. What EFS does **not** yet have is an on-chain *blessed-lens registry* / on-chain lens lists (ADR-0031 explicitly deferred "no on-chain edition lists: the URL is the list"). Whether the multi-app vision needs that primitive in v1 is a protocol decision, not an SDK one — **surfaced to James** (see Open Questions Q6).
...
- [ ] **Q6 (canonical shared config / on-chain blessed-lens registry) — NEW, surfaced 2026-05-28 by the multi-team review.** For unrelated teams to *agree* on shared config, some lens must be canonical, but lenses are per-reader (no global truth). Off-chain has ADR-0039's `systemLenses[]` default tail; on-chain a contract must pass an agreed curator/system lens explicitly (`[EFSRouter.DEPLOYER()]` or a registry-published address). ADR-0031 deferred on-chain lens lists ("the URL is the list"). **The fork is protocol-level, not SDK:** is "read canonical config through an agreed system lens, by convention" sufficient for the multi-app vision in v1 **(a)**, or does EFS need an on-chain blessed-lens / lens-list registry primitive sooner **(b)**? PM rec: **(a)** for v1 — convention + the existing systemLenses tail is enough to ship OnionDAO; promote (b) to its own ADR thread if a real cross-team-config use case demands it. *This does not block promotion of this SDK doc; it is an EFS-protocol question the doc now names rather than silently omits.*

**Status: two open forks — Q1 (Solidity library repo home) and Q6 (canonical-config primitive, protocol-level, non-blocking) — otherwise resolved.** Q2–Q5 resolved. Held at `#status/review` per design mode; not self-promoting. James's calls: (1) the Q1 fork, (2) promote vs. revise, and (3) the Q6 posture (a/b) — though (3) is an EFS-protocol decision the SDK doc only flags.
...
**2026-05-28 (entry 2 of 7) — Q1–Q5 resolved (James), folded in one pass.** Q1 (single `sdk/` repo) and Q2 (resource-oriented namespaces) confirmed as already designed. Three design changes (*Q1, Q3 and the Q5 mechanism order below were later superseded — see entries 3 and 4*):
...
**2026-05-28 (entry 4 of 7) — on-chain/off-chain reframe (James clarification).** James clarified two framing errors carried since the PM brief: (1) the **on-chain SDK is a Solidity deliverable** — a *library* (+ inheritable base) that smart-contract devs use *from their own contracts*, not a TypeScript package; and (2) **"off-chain SDK" just means "the TypeScript SDK"** — it has nothing to do with The Graph / a packaged indexer. Changes folded in:
...
**2026-05-28 (entry 5 of 7) — expert review + brainstorm on the reframe (2 agents: Solidity/EAS fidelity, SDK coherence + on-chain ergonomics).** The fidelity agent confirmed the attester thesis is **correct** (an `internal` library inlines into the consuming contract; even a delegatecall-linked library preserves `msg.sender` — only a plain `CALL` to a separate deployed helper breaks it) and the Solidity signatures are sound. Required fixes (folded in):
