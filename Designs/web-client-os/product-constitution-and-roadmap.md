...
| WCOS-R13 | Raw data survives View failure. | A missing, rejected, or crashed Presentation/App never changes underlying Record, File, Project, Release, or Artifact truth. Trusted raw/Files surfaces remain reachable while the host responds; renderer/process/origin failure reconstructs them through the independent rescue path after restart. |
...
| WCOS-R24 | Links nominate; only recognized local intent may launch. | Query/fragment/module/profile input can request but cannot prove Launch, persist configuration, install code, grant authority, expose private state, or force full-OS boot. A trusted host `OPEN`, retained exact policy or matching agent mandate may launch a completely verified confined guest under an exact all-denied local grant decision; external navigation/redirect/restore, passive unfurl/inspect/prefetch and ambiguous provenance remain `ACTION_REQUIRED`, and every host authority or material runner residual requires its own ceremony. |
...
| WCOS-R66 | A named App plus resource is a direct boot route with minimum time to qualified data. | Exact App selection/critical-closure retrieval and the initial Reader request overlap where safe; no catalog search, wallet, account, Commons, Data Explorer intermediary, private-profile hydration or full Session Shell is required. The Minimal App Host supplies trusted progress, launch/exit and raw fallback, and the App receives the same pinned qualified result. |
| WCOS-R67 | Third-party execution uses an honest practical runner gradient. | An OS-owned outer SES Compartment in a dedicated Worker is the leading confined-JavaScript candidate, with LavaMoat/Endo only as inner dependency hardening; opaque-origin iframes serve full-DOM Apps; Wasm/WIT serves portable non-DOM modules. Link intent never grants authority, zero grants never erase runner residuals, updates inherit neither grants nor state, and every lane has exact construction, lease, teardown and hostile-fixture evidence. See [[app-runtime-and-direct-launch]]. |
...
| **Near-term Web Client** | Rename/move and delete/whiteout after semantics mature; wallet and smart-account adapters; generation-safe PWA offline shell and exact retained-resource replay; first complete additional production translations plus verified inert installable/updateable language-pack generations; verified media ranges; folder Presentations; generic exact/follow App links through a Minimal App Host; first SES Worker and opaque-frame runtime fixtures; Arcade detail/Play; package consumption; local settings and handler bindings; first private overlays; read-only exact/follow system-profile Inspector and metadata adoption/export | General-purpose multi-user OS, automatic/forced updates, background sync without proven recovery, unconstrained runners, shared-profile execution or a production arbitrary-App safety claim before hostile fixtures pass |
...
### Slice D — passive media and first optional executable modules

First add verified ranges for large passive content without weakening
whole-closure verification for executable artifacts. Then run the generic
three-lane proof: one exact read-only SES Worker App over a host-semantic
surface, the same bounded operation through a Wasm Worker, and Arcade as the
first concrete opaque full-Web/product pressure test—an inert Project detail,
exact Release and closure, explicit Play, complete verification, one bounded
legacy HTML profile, teardown, and no host-endowed wallet/EFS capability. Do
not make any of these part of Files correctness or call the proof a production
arbitrary-App safety certification.
