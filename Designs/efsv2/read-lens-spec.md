...
> **2026-07-28 REPLACED as entry point.** The dedicated lens pass ran ([[lens-pass-synthesis]]); **[[lens-spec]] is the successor routing spec.** Every section of this document is dispositioned still-valid / changed / superseded in [critic §3.1](../../Reviews/2026-07-25-lens-pass-corpus/critic.md) — consult that table before citing anything here. Highlights: anti-fallthrough, deny-after-resolve (§3.4), the expiry context split, follow budgets, the classifier/prefix grammar (§6.1–6.4), LC1–LC6, and the §9.B install walkthrough survive re-typed; the flat lens input, the §2 grade dominance ladder, EQUIVOCAL/CONTESTED, checkpoint-grounded absence (§5.2's absence role), MUST-pull-home (§5.4), and the `?lenses=`/`?deny=` link keys are superseded.
>
> **Tier: REOPENED DURABLE DRAFT (historical).** This document is historical design input for the replacement reader spec. It is **not** Etched kernel surface and is not currently a complete conformance authority. Its proven-absence, fail-closed, basis, and scoped-combiner insights should survive; its flat lens input, global same-order equivocation, old KEL grades, and pinned dependency claims must not.
>
