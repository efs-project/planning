...
### 5. The mission ruling — RULED (James, 2026-07-02)

**Default permanence vs a free ephemeral tier.** The judges' split reduced to this: records-first architectures win only if EFS serves sub-$0.001 stranger-writes (comments/social/reviews) via a free tier — which is precisely the tier that is "a publishing medium, not an archive" (Nostr-grade, dies in years 3–10, evidentiary-dead post-CRQC if never anchored).

**Ruling: EFS is a permanent archive — everyone pays for the data they write to the chain; there is no free ephemeral tier.** Every write is on-chain and permanent by default. This closes the mission question and, with it, the records-first architectures (C/D as full systems): their reason to exist was the free tier, which is now out of scope by ruling.

**Sponsored writes take the community-relayer form, not corporate sponsorship.** Cheap/social writes are made affordable by *optional, opt-in* relayers: any anchor, dataset, or community may run a relayer that submits and pays gas for records authored by others. The author stays the record's signer (the envelope recovers author from signature, §2), so sponsorship is purely additive — a community can start or stop paying without affecting anyone's identity or data. No protocol privilege, no shared attester. This is the *good* version of "sponsored L2 gas" the earlier draft gestured at; it is a natural consequence of §2, not a new mechanism.
...
3. **Illegal content & operator liability — protocol neutrality RULED (James, 2026-07-02).** The protocol does **not** police content: lenses mean readers see only what they opt into, immutability is core, and there is no protocol-level delete/allowlist (consistent with ADR-0056, viewer sovereignty). The *residual* is not a protocol question — it is an **operator-facing doctrine note**: lenses control what a reader *sees*, but not what a node/mirror *physically stores and serves*. On-chain bytes (SSTORE2) live in every full node's state regardless of anyone's lens; off-chain mirrors push exposure to their hosts. Deliverable is honest documentation for infrastructure operators about what running EFS means per storage tier — not a protocol change. (Open, unrelated: the "Harberger public option" James raised may reshape the lens/visibility model — needs its own design note when defined.)
4. **Privacy/HNDL design.** "Privacy possible" has requirements but no design: key distribution, metadata privacy (the public claim graph leaks social graphs even with encrypted payloads), harvest-now-decrypt-later on a permanent chain. Extends the encrypted-file conventions in [[efs-v2-holistic-redesign]] §2.3. **Client-OS pressure (2026-07-07, validated):** the persona-linkage case is a concrete instance of this §6.4 concession — the client's default persona-link convention *actively publishes* the correlation, not just passively leaks it. Private *link-content* disclosure is buildable from reserved parts (salted anchor in the URL fragment + `contentEncryption`/`keyWrap` to a per-link key, G9-compliant) with no new Etched surface, but the **authorship edge + timestamp remain unhideable** — the irreducible boundary of a public verifiable write-graph, not a closable gap. Honest framing to adopt project-wide: **privacy-possible, not private-by-default, never anonymous** — cypherpunk on read/custody, publicly-verifiable-by-necessity on write/graph. See [[client-os-pressure-report]] P9.
...
- [x] **§5 mission ruling** — RULED 2026-07-02: permanent archive, everyone pays their own writes, no free ephemeral tier; sponsorship via optional community relayers (§5).
- [x] **§6.3 content neutrality** — RULED 2026-07-02: protocol stays neutral; residual is an operator-doctrine note, not a protocol change.
- [ ] **Carrier call**: EAS-carried v2+ (default) vs native kernel within the window — decided by the week-3 capacity checkpoint per §8. *(Not urgent; James may leave to the checkpoint.)*
- [ ] **Visibility-TAG mechanism** ([[deterministic-ids]] open q): per-write visibility tags vs kernel-ancestor-walk derivation — the one Phase-0 item with a real cost/capability tradeoff (see §"Decision 5" discussion). Needs a James call.
- [ ] **Remaining Phase-0 rubber-stamps**: typed-literal seed set, reserved virtual-key set, raw-UID container drop — recommendations in [[deterministic-ids]]; confirm.
- [ ] **"Harberger public option"** — James to define; may reshape the lens/visibility model (its own design note).
