...
[[ethereum-standards-and-execution-profile]] retains the standards requirements
and execution/read/signature pressure from the August readiness work. Those
requirements remain inputs to the current candidate; that document's dated
`EXP-C0` defaults do not supersede later owner direction or the September
MVP-C0 and programmable-acceptance drafts.
