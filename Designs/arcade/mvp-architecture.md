...
> **Post-pass correction (2026-08-08):** the `contracts/packages/nextjs` placement, broad Sepolia seed, and 12–18-game scope below are initial-pass proposals, not the current implementation baseline. Reconciliation must first compare a lightweight standalone Arcade package against the forthcoming cypherpunk-OS client, keep Catalog/Release/Byte/Runner/Comment sources behind adapters, and prove one Andromeda release without a durable chain write.
