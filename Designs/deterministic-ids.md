...
- [ ] **DATA/LIST duplicate policy × replication model** (coupled — §6/§9): confirm REVERT-for-owned-kinds **and** model A, or adopt model C and re-adjudicate the duplicate branch (idempotent no-op + SDK salt-reuse detection). One joint Phase-0 decision.
