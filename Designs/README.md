...
**Design visibility:** ordinary designs live on `main`, not in agent-specific branches. The [2026-09-10 consolidation map](../Reviews/2026-09-10-design-consolidation.md) records recovered design sets and the exact prototype/evidence sources left in place.
...
| **[`sdkv2/`](./sdkv2/) — EFS v2 offchain and onchain SDK experience** | `planning`, `sdk`, `contracts`, `client` | Founder-authorized draft spine for a raw-preserving offchain runtime, exact generated TypeScript and Solidity facades, compatibility/result/security policy, a dated modern Ethereum standards/profile census, and a source-locked `EXP-C0` disposable MVP handoff. No bytes, package names, ABI, helper deployment, experiment execution, or production implementation is adopted. |
| **[`data-explorer/`](./data-explorer/README.md) — general typed-data workspace** | `planning`, `client`, `sdk` | Recovered full product/research spine with September routing: a separate workspace and Inspector/view consumer of the shared SDK. File Browser works independently; older Explorer-default and August fixture framing are dated proposals, not extra MVP0 gates. |
