...
| **[`web-client-os/`](./web-client-os/) — fast Web Client and modular user-owned Web OS** | `planning`, `client`, `sdk` | Active product-layer spine: route-shaped guest boot and generic direct-to-specific-App links with minimum time to qualified data; official basic File Browser writes; `BIOS -> Kernel -> Shell -> Apps`; standards-first Signals/Web Components application foundation with a mandatory pinned modern-Web implementation-evidence gate; complete pinned EIP/ERC map for exact reads, explicit wallet/provider selection, signatures, URI/content, contract realization, privacy and agent interoperability; practical SES Worker + LavaMoat/Endo inner-dependency, opaque iframe and Wasm/WIT runner lanes; finite exact-Type consumer adapter with raw-preserving generated codecs and stable UI/agent DTOs; responsive installable/offline/global-use profiles; exact linkable system generations with inert inspection and local activation/rollback; user-replaceable service slots; privacy reserves; and human/agent parity. Core/Files/Type names, route/profile bytes, exact dependencies/module/capability ABI, repository, and implementation remain proposal-only. The July `clientv2/` set is historical evidence. |
