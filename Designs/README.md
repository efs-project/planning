...
| **[`data-explorer/`](./data-explorer/) — guest-first typed EFS Data Explorer** | `planning`, `client`, `sdk` | Owner-directed product lane for the primary general-purpose EFS data application: desktop-grade Files navigation plus qualified table/card/gallery/timeline/graph/raw/provenance views over any accepted typed resource. Recommended as a trusted built-in App over shared Reader/Files/action seams, with private local workspace state, lossless raw exit and removable capability-limited extensions. No product repository, implementation, view/query/plugin ABI or protocol bytes are adopted; a read-only slice waits on the guest Files/table/provenance/hostile-data labs, while executable-extension and write lanes have separate later gates. |
