...
One user = **one primary author** (the identity anchor, per the account doctrine) + N **personas**: independent secp256k1 burner authors created, held, and policied by the Kernel. Personas are how EFS gets promptless writes and attenuation *without* protocol session keys: the attenuation lives in *which key* signs, and the policy lives in the Kernel that holds it. The persona system has **two orthogonal axes** — do not conflate them (the correction James drew out, 2026-07-07):

**Axis 1 — key granularity (a *security* choice: how many keys, along which boundary).** Two good defaults that compose:
- **Per-device keys** are a near-necessity: phone and laptop each hold their own key, never synced (a stolen device must not expose the others; each signs locally). A new device is enrolled *from* an existing one (the primary signs its membership once; the primary key never touches the new device).
- **Per-app keys** are a strong default for **blast-radius containment**: a misbehaving app can only ever get *its own* key to sign, so its junk is authored by a burner you can **disavow in one move** (burn the key → its whole history de-correlates from you), never smeared across your main author. The cost is only management + gas, both absorbed by the Kernel holding the keys and relayed submission.

**Axis 2 — linkage visibility (a *privacy* choice, independent of Axis 1: can the world tell it's you?).** Chosen per persona:
- **Public link** → rendered as your one name (device/agent keys; a public app persona). "Everyone knows it's me."
- **Private link** → provable to chosen readers only (§Persona privacy).
- **Unlinked** → nobody knows (an anonymous forum account).

The axes compose freely: *"my phone + laptop + notes-app keys, all shown as james.eth"* **and** *"my anonymous-forum key, unlinked"* are the **same roster at once**, not a mode you switch between. James's "device/agent keys" and "hidden app keys" are two points on Axis 2, both present together — different but related, exactly as intended.

- **Authority isolation** between apps comes from the **Kernel's per-app policy** (scopes/budgets/kinds, below); a per-app *key* adds *reputation*-blast-radius containment + clean disavowal on top of that, and *un*-correlation only if also unlinked (Axis 2). Apps never touch keys — the cage holds at any granularity.
- **Personas sign promptlessly** inside Kernel-enforced policy. No System Chrome prompt per write; the journal accumulates, the outbox shows everything, the ceremony happens at flush checkpoints for the aggregate. The primary itself signs only at consequential checkpoints (§What deserves the primary).
