...
One user = **one primary author** (the identity anchor, per the account doctrine) + N **personas**: independent secp256k1 burner authors created, held, and policied by the Kernel. Personas are how EFS gets promptless writes and attenuation *without* protocol session keys: the attenuation lives in *which key* signs, and the policy lives in the Kernel that holds it. The persona system serves **two different needs with one primitive** — do not conflate them:

- **The default need: one public identity, many device keys.** Most users want to *be themselves* everywhere. Each device holds its own signing key (never sync a key across devices — a stolen laptop must not expose the phone; each signs locally), the primary links and labels them `device:phone` / `device:laptop` (§Linking), and **every write from any device renders as the user's one name** — the ENS/EFS name lives on the primary; device keys inherit it through the link. A new device is enrolled *from* an existing one (the primary signs its membership once; the primary key never has to touch the new device). This is the **default** persona configuration. [corrected 2026-07-07 — the earlier "persona-per-app default" over-weighted privacy]
- **The opt-in need: many *un*-linkable faces.** A separate persona per app or per context (work vs personal, a pseudonym) for users who want on-chain *un*-correlation. Chosen deliberately in the persona picker, never imposed; publicly-labeled-and-linked, privately linked (§Persona privacy), or unlinked. One compromised isolated app burns one author word.
- **Authority isolation between apps does not require a key per app.** Even when several apps share a device key, the **Kernel's per-app policy** (scopes, budgets, subtrees — §policy) contains what any one app can make that key sign, and the cage means apps never touch the key itself. A separate *key* per app buys **on-chain unlinkability**, which is the opt-in privacy mode above — not a compromise-isolation necessity. So the many-keys tax is paid only by users who want it.
