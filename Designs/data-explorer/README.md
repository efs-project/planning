...
**Reviewers:** —
**Last touched:** 2026-09-10 — docs-only consolidation; August source dates retained below
...
## Current routing and scope — 2026-09-10

**Current bounded product contract:** Data Explorer is a separate general EFS
typed-data workspace and Inspector/view consumer over the shared SDK. File
Browser remains the first thin guest/write-capable journey and must open exact
Files routes with Explorer absent, broken or uninstalled. Explorer packaging,
default-route policy, rich views, saved-view publication and executable
extensions are outside the MVP0 gate; no permanent packaging or default App is
selected here.

This retains the September contraction from
[`aae282d`'s concise Explorer spine](https://github.com/efs-project/planning/blob/aae282df72f214d791963aeac1cf3c38d162c56e/Designs/data-explorer/README.md)
and the [[Reviews/2026-09-04-explorer-mvp-convergence/README|September 4 convergence review]].
For the bounded September profile, consume [[Designs/sdkv2/mvp-interface]],
[[Designs/efsv2/disposable-mvp-profile]] and
[[Designs/web-client-os/mvp0-acceptance]] under their own source/evidence ceilings.
[[Designs/sdkv2/README]] remains the current SDK routing authority; importing
this Explorer corpus does not replace its newer direction with August C0.

Explorer owns navigation, tabs, selection, compare state, local layout, view
choice and safe presentation. It may present shared action intent and outcomes,
not a second resolver, verifier, planner, wallet flow, submitter or read-back
stack. Exact identifiers, raw bytes/handles, domain/basis, coverage, support,
validation, authority, currentness, finality, integrity, availability,
returned-byte state, effect and causal evidence remain reachable through the
shared Inspector; views may summarize them but never upgrade or recompute them.

**Historical corpus below:** the eight August design documents are preserved
from [`codex/data-explorer-pm` at `df0ddd3`](https://github.com/efs-project/planning/tree/df0ddd3b77fad7dae4e84c3cde6b009d703cc0cb/Designs/data-explorer).
Their roadmap, broader Explorer production gates and legacy result/wire names
are retained proposals and research history, not additional MVP0 requirements.
August `EXP-C0/v0` and September MVP-C0 are different disposable profiles;
retain the former's preservation laws and raw evidence, not its bytes as the
latter's ABI. Original claims of “current,” “next,” or an experiment pass below
are dated to that source. The August static checker proves neither E1a nor E1b;
no test, browser, participant or production claim is upgraded here.

The [[Reviews/2026-08-25-data-explorer-exp-c0-consumption/README|August packet README]]
locates its scripts and JSON fixtures at the exact preserved source. Only
Markdown design/review documents are consolidated here; no prototype or code
is moved and this operation authorizes no new experiment or implementation.
**Owner choices now:** none; the broader product questions remain evidence-gated
in [[Designs/data-explorer/owner-decision-inbox]].

## August product framing — retained history
...
truth, install themselves, inherit authority or block raw access. The August framing proposed Data Explorer
as the default App and raw fallback for unqualified Files/data routes. That
default is not adopted: the current routing block above keeps File Browser and
exact App routes independent of Explorer and leaves default-route policy open.
