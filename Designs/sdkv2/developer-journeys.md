...
| Build a direct guest inspection/Data Explorer fixture inside the File Browser slice | Compose protocol/Files reads with an exact generated consumer adapter and reduce stable product DTOs/outcomes; offer raw inspection through a deliberate evidence handle. | This is pressure evidence for the owner-directed File Browser, not a parallel product boundary. Ordinary UI never imports Type descriptors/codecs, recomputes identity, selects providers, or treats an indexer as authority. |
