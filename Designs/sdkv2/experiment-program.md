...
| **S0C — C0 shared-envelope preservation** | Can the first disposable adapter preserve the sealed C0 `ResultV0` axes, cursor coordinates, receipt separation, raw evidence, and projection integrity without selecting bytes or Core ABI? | [[Reviews/2026-08-25-sdkv2-exp-c0-mvp/README|Source-only six-case fixture]], field/axis checker, and an explicit source lock to the sealed C0 packet | Every case retains all ten profile axes and required outer groups; cursors include Type/Profile/generation/Realm revision/order/high-water/basis/coverage; raw evidence survives; no `EFFECT_REJECTED` canonical value appears; only exact pre/post equality can mark `NOT_COMMITTED_PROVEN` | A product or generated facade creates another outer outcome law, loses raw bytes, resumes a mismatched cursor, conflates any receipt, treats transport acknowledgement as effect, or presents local rejection as a fourth canonical effect value |
