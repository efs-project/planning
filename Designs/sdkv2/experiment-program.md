...
measurement so a disappointing result cannot move the goalposts. Fixture caps
such as a page count, the current 512-request harness budget, or one storage
span are never promoted into product limits without representative journey
evidence and the normal Core/owner decision path.

S0C retains the August literal `ResultV0` packet as an exact preservation
control. The current SDK candidate instead uses operation-specific result
families with one non-loss qualification/evidence contract; experiments must
not turn either representation into a public freeze.
...
| **S0C — C0 envelope-fixture preservation** | Can the disposable adapter preserve the sealed August C0 fixture through one exact serialized Core packet without importing Core implementation code, treating its wrapper as the production API, or turning candidate bytes into public authority? | [[Reviews/2026-08-25-sdkv2-exp-c0-mvp/README|Six-case semantic fixture plus clean-room serialized consumer]], exact Core commit and role-neutral receipt, five retained serialized inputs, independent Node/ethers checker, SDK-specific report, and 13 adversarial mutations | Exact committed hashes match; all shared/SDK pointers resolve; Result, Bytes, codec-0 and opaque codec-1 envelopes decode, canonically re-encode, and recommit; max `uint64` stays a decimal string; raw evidence survives; receipt roles stay separate; placeholder/stale/mutated inputs reject; every adoption/deployment flag remains false | A consumer imports Core source/scripts/tests, loses or narrows raw evidence, accepts a stale/placeholder lock, resumes a mismatched cursor, conflates any receipt, treats transport acknowledgement as effect, presents the shared fixture as a universal production wrapper, or presents candidate ABI/IDs as frozen or conformant |
...
| **S3 — raw, portable-evidence and unknown survival** | Can old/offline/browser/server runtimes receive, inspect, store, clone, export, relay and later decode future evidence without trusting an erased language brand? | Nested unknowns through decode, DTO mapping, worker transfer, structured clone, Cache/IndexedDB, JSON escape, server queues and archive export; checked export/restore; stripped qualifiers, forged cursor, mixed basis, unsafe integer and unknown result/profile version mutations | Exact original bytes and context hash survive every declared lossless path; only the checked importer restores an evidence-bearing result; lossy paths refuse evidence-preserving claims; unknown remains distinct from invalid/empty; old clients relay unknown extensions byte-for-byte or refuse a lossy edit | Any default path discards or reserializes unknown bytes while retaining an apparently valid identity/result; a plain object/clone revives assurance; stripped or mixed-basis data becomes empty success; an old edit drops future meaning |
| **S4 — operation-specific qualified results** | Can exact-read, scoped-page, verified-byte, plan/submission and canonical-effect families reconcile direct source, cache, indexer, byte source and Lens contradictions without state collapse or a universal wrapper? | Mutation matrix across present/partial/unknown/absent/masked/conflict/invalid/unsupported, byte outcomes, pages, explicit block-hash basis and coverage; same-basis cursor chains with reorder/duplicate/omit/fork mutations; generated claim assessments for selected value, complete enumeration, proved absence, verified bytes, authority and effect | 100% expected family/fact match; applicable qualifications/evidence survive every projection; positive items survive partial coverage; claim assessment remains bound to its observation; only complete exact negative basis creates `ABSENT_PROVEN`; only that absence or a retained selected-whiteout/policy `MASKED` result enters a protocol negative cache; one cursor chain never crosses basis and detects every duplicate/omission | A family drops applicable evidence; one generic `verified` or outcome controls an unrelated claim; timeout, revert, omission, unsupported version, missing helper, unavailable/tampered bytes, incomplete/cross-basis page, duplicate/omitted row, or local presentation mask becomes empty/false/absent/complete or a protocol negative-cache entry |
...
| **S14 — cross-product semantic-adapter parity** | Can one common lossless adapter serve the Web Client/OS direct Files/shell and the independent general typed-data Data Explorer without merging their product models or semantic drift? | Depend on, do not replace, the reviewed Data Explorer E1b sealed-fake-to-real direct-guest packet and facts crosswalk at exact local-only commit `08bb5f2906191f0d87624d9a6ecc6788a8b2754d`, including a cold-browser real-adapter run, index-free rerun and complete browser/network trace; then run the same hostile Core/Files/query/byte/action fixture through distinct Web/Files and Explorer generated façades; reuse S6 plan/digest cases; compare retained evidence, applicable qualification facts, diagnostics, DTO projections and dependency graphs; compare attempted byte locator/range, commitment, verification method/result, verified byte/range identity and unavailable/tampered/policy-blocked/unknown state; compare exact plan commitment and semantic/profile tuple, roles/authorization basis, effect/receipt commitment and final qualified effect outcome | The direct Explorer trace has zero wallet/account/Commons/indexer/catalog/OS/profile/warm-cache dependency; product DTOs, views, navigation, operation-specific result families and additive presentation fields may differ; every applicable qualification fact remains unchanged and each façade preserves, without replacing, merging or reinterpreting, `UNKNOWN`, `PARTIAL`, `ABSENT_PROVEN`, `MASKED`, `CONFLICT`, `INVALID`, `UNSUPPORTED`, byte state, currentness, coverage, authority and basis; exact raw bytes/identity, byte-verification evidence and action/effect commitments match; Web remains directly useful for Files/shell; Explorer remains useful beyond Files; neither imports Type/Data-ABI machinery, selects a hidden provider or creates a second resolver/verifier | Missing/failed E1b proof; any direct-guest hidden dependency; any raw/evidence loss; qualification, byte-integrity/availability or plan/effect mismatch; omission becomes absence/completeness; either product must adopt the other's state/navigation model; Explorer is reduced to a File Browser panel; Web must boot Explorer; a product/indexer becomes semantic authority; or a second resolver/verifier/result law appears |
...
| **S17 — one integrated Files journey** | Can a declaration-only SDK adapter and parameterized conformance suite extend the pinned Files-screen guest-metadata checkpoint into the one Files path Fable will integrate—without copying its reader—through verified open, plan/submit/reconcile, restart and fresh independent consumption? | Exact pinned input `aae282df72f214d791963aeac1cf3c38d162c56e`; same adapter/fixture for cold guest list, selected File byte/range verification, Lens-before-tag filtering, one supported create/upload, atomic-move preservation only if Core supplies one contract-enforced effect (otherwise explicit refusal), dropped response after the real wallet prompt, destroyed runtime/cache state, checked attempt restore, independently qualified canonical-effect reconciliation, fresh independent consumer, and JSON/clone/storage/plugin handoffs; empty/missing-layout and K10 legacy/new mode/ordinal-domain mutations; human task study plus clean-context agent runs; full request/byte/bundle/parse/latency/memory/prompt/call/transaction/read-back/restart measurements | No second resolver or demo; useful partial positives and coverage survive; unverified bytes never open; no losing File leaks through filtering; every cursor/ordinal/profile mismatch rejects; exactly measured wallet prompts and no automatic retry; restored evidence reconciles per effect and `COMMITTED` appears only after the named verifier succeeds; a fresh consumer reconstructs from declared state/carriers; human and agent both complete the documented flow while assurance remains reachable; component K10/storage results stay separately labelled | The browser must bypass the adapter; SDK copies the fixture reader or UI; a language brand survives only in-process; a lost response becomes failure/retry; K10 or a storage arm is adopted from an isolated number; fixture caps become product limits; human success is inferred solely from an agent; byte, write, read-back, restart or fresh-consumer stages remain independent demos rather than one path |

At the retained pin, only S17's guest metadata-list stage has executable
...
15. Operation-specific results cannot preserve one common qualification/
    evidence contract, or a generated assurance check can detach its answer
    from exact subject, observation, basis, coverage and evidence.
16. Plain JSON/clone/cache/plugin data can regain trusted status without a
    checked import, or an old client cannot preserve unknown extensions or
    refuse an unsafe lossy edit.
17. Files filtering can precede qualified Lens selection; physical ordinals or
    cursor modes cross domains; or missing layout/source expectations can pass
    because a collection is empty.
18. Ambiguous submission cannot be restored and reconciled per effect before
    retry, or the claimed Files experience has not passed S17 as one path with
    both real-human and clean-context-agent evidence.
...
### Week-1A — integrated Files adapter and recovery packet

Write declarations and parameterized assertions—not another reader—for the one
Files path Fable will integrate, starting from the pinned Files-screen checkpoint.
Exercise guest list, verified open, Lens-before-filter,
one planned mutation, dropped-response restart/reconcile, checked portable
restore and a fresh independent consumer through the same adapter. Run both a
real-human task study and clean-context agent probes; measure the entire journey
and keep all K10/storage/component figures separately labelled.

**Exit artifact:** one S17 packet with dependency trace, portable fixtures,
fault injections, prompt count, per-effect recovery, human/agent observations,
whole-journey cost and an explicit list of any missing Core semantics. Delete
the lane if it requires a copied resolver or a second product demo.

### Week-1B — compiler contract spike
...
### Week-1C — Solidity three-arm cost and attack spike
...
### Week-1D — result, basis, and reconstruction harness
