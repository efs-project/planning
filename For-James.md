...
- **3. Q6 — canonical shared config (NON-BLOCKING, protocol-level not SDK).** For unrelated teams to agree on shared config, some lens must be canonical, but lenses are per-reader. **(a)** convention only ("read through an agreed system/curator lens" + ADR-0039's systemLenses tail) is enough for v1/OnionDAO; **(b)** EFS needs an on-chain blessed-lens registry sooner → its own ADR. The SDK doc only *flags* this; it's a contracts call. *(PM rec 3a — doesn't gate the SDK.)*

PM rec: **1a + 2 promote + 3a.** (Expert-review + brainstorm round on the reframe is done — findings folded in, revision log entries 5–6. Nothing else blocking your call.)
