...
**0b. ✅ DECIDED (2026-05-31): frozen schemas + upgradeable contracts.** You chose it — captured in [[Decisions]], supersedes ADR-0030's mainnet "no-proxies" stance. The contracts repo needs a **superseding ADR** documenting the new model (the freeze/deploy thread writes it). **One sub-question remains for that thread (not now):** who holds the proxy-admin upgrade keys, and with what guardrails (multisig / timelock / announced-upgrades / eventual burn)? — that's what makes "upgradeable" trustworthy-enough vs. a rug risk. Flagged on the Kanban freeze card; connects to the SDK systemLenses-trust-root flag.
