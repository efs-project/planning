...
**0. 🟢 PR #20 (EFS Lists) is GREEN + CLEAN — merge when you're ready.** All 3 CI checks pass (dev cleared prettier + Alchemy). PM review was advisory GO; merge is your call (agent reviews aren't sign-off). **This merge is the unblock for the schema freeze you want ASAP** — schemas finalize with it. After merge: spawn the freeze/deploy thread (PM will draft the prompt on your word).

**0b. ✅ DECIDED (2026-05-31): frozen schemas + stable resolver addresses (proxies) + upgradable logic; contract set stays flexible** (add freely, remove cautiously, pre-mainnet). Captured in [[Decisions]]; supersedes ADR-0030's mainnet "no-proxies" stance → contracts repo needs a superseding ADR (freeze/deploy thread writes it). **One sub-question for that thread (not now):** who holds proxy-admin upgrade keys + guardrails (multisig / timelock / announced / eventual burn) — what makes "upgradeable" trustworthy vs. a rug risk; connects to the SDK systemLenses-trust-root flag.
- **Tier-1 flag (needs your OK):** the Milestones OnionDAO list still says *"Smart contract .sol file list freeze"* — now superseded by your "set stays flexible" clarification. Want me to update that Milestone wording? (Milestone scope edits are yours to approve.)
