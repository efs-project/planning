...
**0. ✅ PR #20 (EFS Lists) — PM-reviewed, ready to merge.** Full findings posted to the PR (advisory review, GitHub). Substantively sound: design faithful to ADR-0044/0046, no BLOCKING/HIGH. Only blockers are 2 trivial CI fixes the **dev** owns (prettier `lint:fix`; Alchemy-key rotation). **Your part:** after merge, fold the 3 §8 deploy items (CREATE2-vs-CREATE, hardcoded schema-UID assert, document `_listAttesters`) into the upgrade-strategy / Sepolia-deploy thread — they ARE that thread's opening agenda. Detail on PR #20.

**0b. ⚠️ Architectural contradiction to decide deliberately (NOT OnionDAO-blocking — gates the post-merge freeze/deploy thread).** The dev's schema-freeze plan makes "frozen schemas + **upgradeable contracts behind stable proxies**" the backbone. But **ADR-0030 (Accepted) says the opposite for mainnet**: *"No proxies, no upgradeability, no admin escape hatches. No admin-anything."* The dev framed this as "a deviation to reconcile" — it's actually a **head-on reversal of an Accepted ADR about EFS's core credible-neutrality guarantee** (ADR-0030's thesis: anyone with admin keys can retroactively change rules / censor data). This needs your explicit call + a **superseding ADR**, not a quiet checklist line.
- **Sepolia/OnionDAO is unaffected** — ADR-0030 *explicitly permits* upgradeable proxies on devnet/Sepolia. So the upcoming deploy proceeds fine; this is purely a *mainnet* decision.
- **The fork (mainnet):** (a) stay immutable per ADR-0030, accept the tail risk (critical bug = redeploy + data loss), "get the launch version right"; (b) supersede ADR-0030 → upgradeable proxies, but then you MUST define **who holds the proxy-admin keys** (multisig? timelock? burned-after-stabilization?) — because that reintroduces the exact "someone can change/censor EFS" property ADR-0030 rejected; (c) hybrid (e.g. upgradeable behind timelock, admin burned once stable).
- **Connects to** the SDK design's flag that systemLenses trust roots should be immutable contracts/multisigs not EOAs — "who holds keys that can change EFS" is now surfacing in two places; worth one unified answer.
- **PM rec:** don't decide under launch pressure; queue it as the freeze/deploy thread's opening decision. Also note: this **closes my earlier §8 CREATE2 finding** — the dev's nonce-CREATE-on-devnet → CREATE2-at-mainnet plan (with the assert-predicted-address + UID back-read scaffolding) is a deliberate staged plan, not a gap. Canonical home for the freeze checklist = **contracts repo** (ADR + `docs/` checklist), per our design-location rule — see my note to the dev below.
