...
**0. ⚠️ Consider pushing the Lists WIP branch.** Status resolved (2026-05-30): Lists schemas done, dev fixing bugs — **but it's all LOCAL/unpushed.** No remote branch has the resolver `.sol` files; newest push anywhere is `custom-lists` @ 2026-05-28 (ADRs/design only). Risk at T-2: the schema-freeze thread, SDK thread, and PM are all blind to the real schema UIDs + resolver addresses they must coordinate against; and unpushed work is a single-machine-failure point. **Not urgent-blocking, but a WIP push (even messy) would unblock parallel threads.** Your call — it's the dev's branch.
