...
**0. ✅ PR #20 (EFS Lists) reviewed — substantively READY; 2 tiny fixes + 3 conscious calls before merge.** PM ran 3 subagents (design-fidelity, code-correctness, CI-diagnosis). Verdict: **design FAITHFUL to ADR-0044/0046; code correct (no BLOCKING/HIGH — reviewer tried to break it, couldn't); CI reds are cosmetic.**
- **Before merge — 2 trivial:** (a) `ci` fails only on **prettier formatting** (15 auto-fixable warnings in 3 test/script files) → `lint:fix` + commit. The real freeze-invariant + schema-UID checks PASSED. (b) `deploy-pin-check` fails = the **known Alchemy-key rotation** (non-blocking per ADR-0028; fix via [[known-issues]] playbook, 2 files).
- **3 conscious calls (these feed directly into your "upgrade strategy + real Sepolia deploy" next step — see below):**
  1. **CREATE2 → CREATE drift.** ADR-0044 §8 mandates CREATE2-with-salt; impl uses nonce-based CREATE, and ADR-0046 re-justifies it as "nonce-deterministic on the pinned fork." Defensible pre-mainnet, but it's a deviation from an Etched ADR. Accept + amend §8, or ask for CREATE2.
  2. **No hardcoded schema-UID equality assert.** The CI "pin check" guards `deployedContracts.ts` drift, NOT `assert(LIST_ENTRY_SCHEMA_UID == expected)` as §8 asked. This is *exactly* the guarantee that keeps Sepolia↔mainnet from diverging — so it matters most for the deploy strategy.
  3. **`_listAttesters` on-chain index is undocumented** in any ADR Decision section — load-bearing append-only state shipping without a spec home. Wants a sentence in an ADR before its address freezes in.
- **PM rec:** fixes (a)+(b) now → merge; fold calls 1–3 into the upgrade-strategy/deploy thread (they ARE that work). Low findings (view-overflow on `start+len`, open-curation `_listAttesters` bloat, `curator` naming) are non-blocking — captured in [[Decisions]].
