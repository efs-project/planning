...
**f) Update Owner-Inbox.md and Decisions.md if relevant.** If the design's resolution should be discoverable as a one-line decision (e.g., "we now use lexicographic-by-attester-address as the tiebreaker"), add a line to `Decisions.md`. If the landing freed up a `#needs/owner` item in `Owner-Inbox.md`, check it off.
