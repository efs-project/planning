...
- [x] **🧅 BUILDATHON KIT READY — "The Forever Files"** (@oniondao, 2026-06-23) — flyer.html + announcement + Discord pinned + submission/dataset templates + judging rubric + 4 sample datasets, in the `hackathon/` repo. Single dataset track, $195 cash prizes, Jun 23→Jul 8. Participant path LIVE (Sepolia explorer #30 + faucet #31 + minimal-clicks writes #36). At-handout gate is James-manual (publish eth.limo URL, set up Discord, fill 2 links).
- [x] **Debug explorer → live Sepolia** (@client-sepolia) — Sepolia + 3-network switcher (PR #30), gas-drip faucet (#31), minimal-clicks batch writes via layered multiAttest (#36), browser read caching (#38) — all merged 2026-06-23. The self-serve participant UI is live.
