...
- [ ] **Schema freeze + upgrade-strategy thread (post-Lists-merge)** — dev sent full design inputs (2026-05-31): frozen EAS schemas + upgradeable contracts behind stable proxies; count-vs-discriminator sizing audit (ADR-0047 did maxEntries→uint256); confirm revocable flags; CREATE2-at-mainnet; adversarial-review hardening carry-overs. **Decision gate: the proxy/upgradeable model REVERSES ADR-0030 (no-proxies mainnet) → needs a superseding ADR + James's call on proxy-admin trust (see For-James 0b).** Canonical home = contracts repo (ADR + `docs/` checklist). PM to draft this thread's launch prompt on James's word / at merge. #repo/contracts #depends-on/lists-merge #blocked-on/human-decision
- [ ] Freeze smart-contract .sol file list — lock the set the moment PR #20 merges (rolls into the freeze thread above). #repo/contracts #depends-on/lists-merge
