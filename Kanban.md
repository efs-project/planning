...
- [ ] **Reconcile and build the one-game [[Designs/arcade/README|EFS Arcade]] vertical slice** — Fable's broad 2026-08-07 research pass is now on `main`, but its demo-only framing, 12–18-game launch, Norman choice, durable v1 seed, faucet/star/comment-archiver scope, and debug-client placement were superseded by post-pass validation. Current slice: Andromeda Invaders; anonymous metadata then explicit Play; exact Project/Release/ArtifactManifest identity; strict verified locator fallback; a versioned runner policy; and no durable chain write until the greenfield EFS data model freezes. Keep product work behind a provisional adapter. Decide the lightweight debug-client/standalone-package versus cypherpunk-OS timing boundary before implementation. #repo/planning #repo/contracts #repo/client #kind/design #topic/content
...
- [ ] **Reframe the permanent EFS successor as a greenfield design** (@fable / @codex-gpt-5) — James's 2026-08-08 ruling retires v1 support/migration/coexistence and the EAS-backed 1.5 bridge as constraints. The native-envelope/tag-core v2 work, EAS-backed 1.5 pass, v1 validation record, client designs, and use-case reviews are evidence, not competing implementation targets. Preserve the hard-earned requirements—universal deterministic IDs, offline/light-client verification, atomic idempotent writes, honest bounded reads, shared Types/validators, authority separation, and recoverability—while making every mechanism re-earn inclusion. Compare self-contained records with immutable descriptor/envelope normalization across Arcade, Git, Nanda, Markdown, anonymous reads, and shared Topics; derive the minimum record algebra and narrow module boundaries; prototype and benchmark both shapes; then run independent long-horizon review. EAS has no assumed carrier role. No implementation or freeze until James reviews the smaller frame. `Designs/efs15/` remains historical input pending owning-thread integration. #repo/planning #repo/contracts #repo/sdk #repo/client #kind/design
  — @pm reframed 2026-08-08; next: owning design thread produces the compact invariant ledger and architecture bakeoff, then returns only irreducible owner choices
