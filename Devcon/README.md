# Devcon 8 accepted talk

Application record and delivery control center for James's Devcon 8 talk. This
folder is a conference-planning surface, not an EFS design ruling or a promise
to ship v2 by Devcon.

**Status (2026-09-02):** proposal accepted; James confirmed participation; the
public session page is live. Talk delivery, scheduling, visa, and travel
follow-ups remain open.

**Session:** [Who Can Turn Off Your Ethereum App? A Full-Stack Walk-Away Test](https://devcon.org/en/schedule/devcon8/SPVJV8/)

## Deadline record

Two official surfaces disagree:

- The [public speaker page](https://devcon.org/en/speaker-applications/) says
  **2026-08-06 23:59 UTC** (18:59 CDT).
- The [live CFP](https://cfp.devcon.org/devcon8/cfp) says
  **2026-08-06 12:00 Asia/Kolkata** (06:30 UTC, **01:30 CDT**). Its live
  countdown agreed with the earlier time when checked.

The proposal was submitted before the conservative **01:30 CDT on August 6**
live-form cutoff. The discrepancy is retained here as process evidence.

Historical direct form: [start a proposal](https://cfp.devcon.org/devcon8/submit/).

## Submitted application

- **Title:** Who Can Turn Off Your Ethereum App? A Full-Stack Walk-Away Test
- **Track:** Permissionless Networks
- **Format:** Talk — 20 minutes plus 5 minutes Q&A
- **Tags, in order:** Censorship Resistance; Decentralization; Open Source
- **Thesis:** a content hash or decentralized carrier does not by itself make an
  application durable. Teach a layer-by-layer walk-away test for operator-owned
  dependencies, using live EFS on Sepolia and its independently reproducible
  proof as the case study.
- **Positioning rule:** EFS is the evidence, not the product being promoted.

The submitted answers are in [[application-draft]]. Research and provenance are
in [[speaker-application-research]]. Alternatives and the reasoning behind this
choice are in [[talk-concepts]]. The next-conference date watch is in
[[ethdenver-2027-watch]] and remains separate from this proposal.
Ticket, visa, travel, and EFS participation follow-ups are in
[[attendance-checklist]]. Keep sensitive travel and identity details out of this
repository.

## Acceptance completed

The proposal was submitted before the conservative live-form cutoff. The final
public page, on-screen confirmation, and review URL are recorded in
[[application-draft]].

Devcon accepted the talk in the **Permissionless Networks** track as a
**20-minute talk plus 5 minutes of Q&A**. James confirmed participation on
2026-09-02. Do not add private acceptance email content, signed-in speaker
portal links, or confidential visa-support material here. Record only resulting
public-safe status and actions.

## Owner confirmations

- [x] Devcon attendance: 3+.
- [x] Telegram or alternate contact supplied.
- [x] MC interest: no.
- [x] Public biography approved.
- [x] Public display name and square profile image approved.
- [x] CFP account/login completed.
- [x] Proposal submitted, not merely saved as a draft.
- [x] Proposal URL and on-screen confirmation recorded below.
- [x] Proposal accepted and participation confirmed.
- [ ] James personally reviews the official Speaker Guidelines.

**Public record:** [accepted Devcon 8 session](https://devcon.org/en/schedule/devcon8/SPVJV8/); acceptance and participation confirmation recorded 2026-09-02.

## Contact boundary

Contacts may provide honest fit feedback, but should not be asked to pressure a
reviewer or bypass the public process. The official speaker contact is
[speak@devcon.org](mailto:speak@devcon.org). Current track leads and reviewers
were not publicly listed during the research pass.

## After acceptance

- The exact session date, time, room, and presentation logistics have not been
  announced. Keep November 3-6 available and record official updates when they
  arrive.
- Follow [[attendance-checklist]] now. The private confirmed-speaker portal
  supersedes the pre-acceptance visa inquiry. Its confidential support intake was
  completed on **2026-09-02**, before the 2026-09-29 deadline.
- Keep the session educational and non-promotional. EFS remains the concrete
  case study for a generally useful walk-away test.
- Build the talk around preverified records and an offline recording. Never make
  conference Wi-Fi, a faucet, a fresh transaction, or one RPC the demo's single
  point of failure.
- Preserve the boundary: v1 is live Sepolia evidence; v2 and the cypherpunk OS
  remain active design work unless their status changes before the talk.
